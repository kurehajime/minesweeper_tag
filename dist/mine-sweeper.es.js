var Rs = Object.defineProperty;
var Ds = (A, e, t) => e in A ? Rs(A, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : A[e] = t;
var me = (A, e, t) => (Ds(A, typeof e != "symbol" ? e + "" : e, t), t);
function Be(A, e) {
  const t = /* @__PURE__ */ Object.create(null), o = A.split(",");
  for (let n = 0; n < o.length; n++)
    t[o[n]] = !0;
  return e ? (n) => !!t[n.toLowerCase()] : (n) => !!t[n];
}
function Ct(A) {
  if (Q(A)) {
    const e = {};
    for (let t = 0; t < A.length; t++) {
      const o = A[t], n = AA(o) ? Vs(o) : Ct(o);
      if (n)
        for (const s in n)
          e[s] = n[s];
    }
    return e;
  } else {
    if (AA(A))
      return A;
    if (b(A))
      return A;
  }
}
const Us = /;(?![^(]*\))/g, Ks = /:([^]+)/, ks = /\/\*.*?\*\//gs;
function Vs(A) {
  const e = {};
  return A.replace(ks, "").split(Us).forEach((t) => {
    if (t) {
      const o = t.split(Ks);
      o.length > 1 && (e[o[0].trim()] = o[1].trim());
    }
  }), e;
}
function fo(A) {
  let e = "";
  if (AA(A))
    e = A;
  else if (Q(A))
    for (let t = 0; t < A.length; t++) {
      const o = fo(A[t]);
      o && (e += o + " ");
    }
  else if (b(A))
    for (const t in A)
      A[t] && (e += t + " ");
  return e.trim();
}
const Js = "itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly", Ts = /* @__PURE__ */ Be(Js);
function Mn(A) {
  return !!A || A === "";
}
const ot = (A) => AA(A) ? A : A == null ? "" : Q(A) || b(A) && (A.toString === Nn || !R(A.toString)) ? JSON.stringify(A, Cn, 2) : String(A), Cn = (A, e) => e && e.__v_isRef ? Cn(A, e.value) : ne(e) ? {
  [`Map(${e.size})`]: [...e.entries()].reduce((t, [o, n]) => (t[`${o} =>`] = n, t), {})
} : xn(e) ? {
  [`Set(${e.size})`]: [...e.values()]
} : b(e) && !Q(e) && !wn(e) ? String(e) : e, L = process.env.NODE_ENV !== "production" ? Object.freeze({}) : {}, Ne = process.env.NODE_ENV !== "production" ? Object.freeze([]) : [], iA = () => {
}, In = () => !1, js = /^on[^a-z]/, We = (A) => js.test(A), pt = (A) => A.startsWith("onUpdate:"), _ = Object.assign, ho = (A, e) => {
  const t = A.indexOf(e);
  t > -1 && A.splice(t, 1);
}, Zs = Object.prototype.hasOwnProperty, V = (A, e) => Zs.call(A, e), Q = Array.isArray, ne = (A) => It(A) === "[object Map]", xn = (A) => It(A) === "[object Set]", R = (A) => typeof A == "function", AA = (A) => typeof A == "string", go = (A) => typeof A == "symbol", b = (A) => A !== null && typeof A == "object", Eo = (A) => b(A) && R(A.then) && R(A.catch), Nn = Object.prototype.toString, It = (A) => Nn.call(A), mo = (A) => It(A).slice(8, -1), wn = (A) => It(A) === "[object Object]", Mo = (A) => AA(A) && A !== "NaN" && A[0] !== "-" && "" + parseInt(A, 10) === A, nt = /* @__PURE__ */ Be(
  ",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"
), Fs = /* @__PURE__ */ Be("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"), xt = (A) => {
  const e = /* @__PURE__ */ Object.create(null);
  return (t) => e[t] || (e[t] = A(t));
}, bs = /-(\w)/g, TA = xt((A) => A.replace(bs, (e, t) => t ? t.toUpperCase() : "")), qs = /\B([A-Z])/g, EA = xt((A) => A.replace(qs, "-$1").toLowerCase()), Nt = xt((A) => A.charAt(0).toUpperCase() + A.slice(1)), Ae = xt((A) => A ? `on${Nt(A)}` : ""), Ve = (A, e) => !Object.is(A, e), Se = (A, e) => {
  for (let t = 0; t < A.length; t++)
    A[t](e);
}, ft = (A, e, t) => {
  Object.defineProperty(A, e, {
    configurable: !0,
    enumerable: !1,
    value: t
  });
}, ht = (A) => {
  const e = parseFloat(A);
  return isNaN(e) ? A : e;
};
let jo;
const Gn = () => jo || (jo = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : typeof global < "u" ? global : {});
function zt(A, ...e) {
  console.warn(`[Vue warn] ${A}`, ...e);
}
let SA;
class Ws {
  constructor(e = !1) {
    this.detached = e, this.active = !0, this.effects = [], this.cleanups = [], this.parent = SA, !e && SA && (this.index = (SA.scopes || (SA.scopes = [])).push(this) - 1);
  }
  run(e) {
    if (this.active) {
      const t = SA;
      try {
        return SA = this, e();
      } finally {
        SA = t;
      }
    } else
      process.env.NODE_ENV !== "production" && zt("cannot run an inactive effect scope.");
  }
  on() {
    SA = this;
  }
  off() {
    SA = this.parent;
  }
  stop(e) {
    if (this.active) {
      let t, o;
      for (t = 0, o = this.effects.length; t < o; t++)
        this.effects[t].stop();
      for (t = 0, o = this.cleanups.length; t < o; t++)
        this.cleanups[t]();
      if (this.scopes)
        for (t = 0, o = this.scopes.length; t < o; t++)
          this.scopes[t].stop(!0);
      if (!this.detached && this.parent && !e) {
        const n = this.parent.scopes.pop();
        n && n !== this && (this.parent.scopes[this.index] = n, n.index = this.index);
      }
      this.parent = void 0, this.active = !1;
    }
  }
}
function Ls(A, e = SA) {
  e && e.active && e.effects.push(A);
}
const Je = (A) => {
  const e = new Set(A);
  return e.w = 0, e.n = 0, e;
}, Bn = (A) => (A.w & XA) > 0, yn = (A) => (A.n & XA) > 0, zs = ({ deps: A }) => {
  if (A.length)
    for (let e = 0; e < A.length; e++)
      A[e].w |= XA;
}, Hs = (A) => {
  const { deps: e } = A;
  if (e.length) {
    let t = 0;
    for (let o = 0; o < e.length; o++) {
      const n = e[o];
      Bn(n) && !yn(n) ? n.delete(A) : e[t++] = n, n.w &= ~XA, n.n &= ~XA;
    }
    e.length = t;
  }
}, Ht = /* @__PURE__ */ new WeakMap();
let De = 0, XA = 1;
const vt = 30;
let uA;
const se = Symbol(process.env.NODE_ENV !== "production" ? "iterate" : ""), Xt = Symbol(process.env.NODE_ENV !== "production" ? "Map key iterate" : "");
class Co {
  constructor(e, t = null, o) {
    this.fn = e, this.scheduler = t, this.active = !0, this.deps = [], this.parent = void 0, Ls(this, o);
  }
  run() {
    if (!this.active)
      return this.fn();
    let e = uA, t = vA;
    for (; e; ) {
      if (e === this)
        return;
      e = e.parent;
    }
    try {
      return this.parent = uA, uA = this, vA = !0, XA = 1 << ++De, De <= vt ? zs(this) : Zo(this), this.fn();
    } finally {
      De <= vt && Hs(this), XA = 1 << --De, uA = this.parent, vA = t, this.parent = void 0, this.deferStop && this.stop();
    }
  }
  stop() {
    uA === this ? this.deferStop = !0 : this.active && (Zo(this), this.onStop && this.onStop(), this.active = !1);
  }
}
function Zo(A) {
  const { deps: e } = A;
  if (e.length) {
    for (let t = 0; t < e.length; t++)
      e[t].delete(A);
    e.length = 0;
  }
}
let vA = !0;
const Yn = [];
function fe() {
  Yn.push(vA), vA = !1;
}
function he() {
  const A = Yn.pop();
  vA = A === void 0 ? !0 : A;
}
function dA(A, e, t) {
  if (vA && uA) {
    let o = Ht.get(A);
    o || Ht.set(A, o = /* @__PURE__ */ new Map());
    let n = o.get(t);
    n || o.set(t, n = Je());
    const s = process.env.NODE_ENV !== "production" ? { effect: uA, target: A, type: e, key: t } : void 0;
    Pt(n, s);
  }
}
function Pt(A, e) {
  let t = !1;
  De <= vt ? yn(A) || (A.n |= XA, t = !Bn(A)) : t = !A.has(uA), t && (A.add(uA), uA.deps.push(A), process.env.NODE_ENV !== "production" && uA.onTrack && uA.onTrack(Object.assign({ effect: uA }, e)));
}
function ZA(A, e, t, o, n, s) {
  const i = Ht.get(A);
  if (!i)
    return;
  let l = [];
  if (e === "clear")
    l = [...i.values()];
  else if (t === "length" && Q(A)) {
    const u = ht(o);
    i.forEach((f, p) => {
      (p === "length" || p >= u) && l.push(f);
    });
  } else
    switch (t !== void 0 && l.push(i.get(t)), e) {
      case "add":
        Q(A) ? Mo(t) && l.push(i.get("length")) : (l.push(i.get(se)), ne(A) && l.push(i.get(Xt)));
        break;
      case "delete":
        Q(A) || (l.push(i.get(se)), ne(A) && l.push(i.get(Xt)));
        break;
      case "set":
        ne(A) && l.push(i.get(se));
        break;
    }
  const c = process.env.NODE_ENV !== "production" ? { target: A, type: e, key: t, newValue: o, oldValue: n, oldTarget: s } : void 0;
  if (l.length === 1)
    l[0] && (process.env.NODE_ENV !== "production" ? Ce(l[0], c) : Ce(l[0]));
  else {
    const u = [];
    for (const f of l)
      f && u.push(...f);
    process.env.NODE_ENV !== "production" ? Ce(Je(u), c) : Ce(Je(u));
  }
}
function Ce(A, e) {
  const t = Q(A) ? A : [...A];
  for (const o of t)
    o.computed && Fo(o, e);
  for (const o of t)
    o.computed || Fo(o, e);
}
function Fo(A, e) {
  (A !== uA || A.allowRecurse) && (process.env.NODE_ENV !== "production" && A.onTrigger && A.onTrigger(_({ effect: A }, e)), A.scheduler ? A.scheduler() : A.run());
}
const vs = /* @__PURE__ */ Be("__proto__,__v_isRef,__isVue"), Qn = new Set(
  /* @__PURE__ */ Object.getOwnPropertyNames(Symbol).filter((A) => A !== "arguments" && A !== "caller").map((A) => Symbol[A]).filter(go)
), Xs = /* @__PURE__ */ wt(), Ps = /* @__PURE__ */ wt(!1, !0), _s = /* @__PURE__ */ wt(!0), $s = /* @__PURE__ */ wt(!0, !0), bo = /* @__PURE__ */ Ai();
function Ai() {
  const A = {};
  return ["includes", "indexOf", "lastIndexOf"].forEach((e) => {
    A[e] = function(...t) {
      const o = U(this);
      for (let s = 0, i = this.length; s < i; s++)
        dA(o, "get", s + "");
      const n = o[e](...t);
      return n === -1 || n === !1 ? o[e](...t.map(U)) : n;
    };
  }), ["push", "pop", "shift", "unshift", "splice"].forEach((e) => {
    A[e] = function(...t) {
      fe();
      const o = U(this)[e].apply(this, t);
      return he(), o;
    };
  }), A;
}
function wt(A = !1, e = !1) {
  return function(o, n, s) {
    if (n === "__v_isReactive")
      return !A;
    if (n === "__v_isReadonly")
      return A;
    if (n === "__v_isShallow")
      return e;
    if (n === "__v_raw" && s === (A ? e ? Vn : kn : e ? Kn : Un).get(o))
      return o;
    const i = Q(o);
    if (!A && i && V(bo, n))
      return Reflect.get(bo, n, s);
    const l = Reflect.get(o, n, s);
    return (go(n) ? Qn.has(n) : vs(n)) || (A || dA(o, "get", n), e) ? l : nA(l) ? i && Mo(n) ? l : l.value : b(l) ? A ? Jn(l) : xo(l) : l;
  };
}
const ei = /* @__PURE__ */ Sn(), ti = /* @__PURE__ */ Sn(!0);
function Sn(A = !1) {
  return function(t, o, n, s) {
    let i = t[o];
    if (PA(i) && nA(i) && !nA(n))
      return !1;
    if (!A && (!dt(n) && !PA(n) && (i = U(i), n = U(n)), !Q(t) && nA(i) && !nA(n)))
      return i.value = n, !0;
    const l = Q(t) && Mo(o) ? Number(o) < t.length : V(t, o), c = Reflect.set(t, o, n, s);
    return t === U(s) && (l ? Ve(n, i) && ZA(t, "set", o, n, i) : ZA(t, "add", o, n)), c;
  };
}
function oi(A, e) {
  const t = V(A, e), o = A[e], n = Reflect.deleteProperty(A, e);
  return n && t && ZA(A, "delete", e, void 0, o), n;
}
function ni(A, e) {
  const t = Reflect.has(A, e);
  return (!go(e) || !Qn.has(e)) && dA(A, "has", e), t;
}
function si(A) {
  return dA(A, "iterate", Q(A) ? "length" : se), Reflect.ownKeys(A);
}
const On = {
  get: Xs,
  set: ei,
  deleteProperty: oi,
  has: ni,
  ownKeys: si
}, Rn = {
  get: _s,
  set(A, e) {
    return process.env.NODE_ENV !== "production" && zt(`Set operation on key "${String(e)}" failed: target is readonly.`, A), !0;
  },
  deleteProperty(A, e) {
    return process.env.NODE_ENV !== "production" && zt(`Delete operation on key "${String(e)}" failed: target is readonly.`, A), !0;
  }
}, ii = /* @__PURE__ */ _({}, On, {
  get: Ps,
  set: ti
}), ri = /* @__PURE__ */ _({}, Rn, {
  get: $s
}), Io = (A) => A, Gt = (A) => Reflect.getPrototypeOf(A);
function Pe(A, e, t = !1, o = !1) {
  A = A.__v_raw;
  const n = U(A), s = U(e);
  t || (e !== s && dA(n, "get", e), dA(n, "get", s));
  const { has: i } = Gt(n), l = o ? Io : t ? No : Te;
  if (i.call(n, e))
    return l(A.get(e));
  if (i.call(n, s))
    return l(A.get(s));
  A !== n && A.get(e);
}
function _e(A, e = !1) {
  const t = this.__v_raw, o = U(t), n = U(A);
  return e || (A !== n && dA(o, "has", A), dA(o, "has", n)), A === n ? t.has(A) : t.has(A) || t.has(n);
}
function $e(A, e = !1) {
  return A = A.__v_raw, !e && dA(U(A), "iterate", se), Reflect.get(A, "size", A);
}
function qo(A) {
  A = U(A);
  const e = U(this);
  return Gt(e).has.call(e, A) || (e.add(A), ZA(e, "add", A, A)), this;
}
function Wo(A, e) {
  e = U(e);
  const t = U(this), { has: o, get: n } = Gt(t);
  let s = o.call(t, A);
  s ? process.env.NODE_ENV !== "production" && Dn(t, o, A) : (A = U(A), s = o.call(t, A));
  const i = n.call(t, A);
  return t.set(A, e), s ? Ve(e, i) && ZA(t, "set", A, e, i) : ZA(t, "add", A, e), this;
}
function Lo(A) {
  const e = U(this), { has: t, get: o } = Gt(e);
  let n = t.call(e, A);
  n ? process.env.NODE_ENV !== "production" && Dn(e, t, A) : (A = U(A), n = t.call(e, A));
  const s = o ? o.call(e, A) : void 0, i = e.delete(A);
  return n && ZA(e, "delete", A, void 0, s), i;
}
function zo() {
  const A = U(this), e = A.size !== 0, t = process.env.NODE_ENV !== "production" ? ne(A) ? new Map(A) : new Set(A) : void 0, o = A.clear();
  return e && ZA(A, "clear", void 0, void 0, t), o;
}
function At(A, e) {
  return function(o, n) {
    const s = this, i = s.__v_raw, l = U(i), c = e ? Io : A ? No : Te;
    return !A && dA(l, "iterate", se), i.forEach((u, f) => o.call(n, c(u), c(f), s));
  };
}
function et(A, e, t) {
  return function(...o) {
    const n = this.__v_raw, s = U(n), i = ne(s), l = A === "entries" || A === Symbol.iterator && i, c = A === "keys" && i, u = n[A](...o), f = t ? Io : e ? No : Te;
    return !e && dA(s, "iterate", c ? Xt : se), {
      next() {
        const { value: p, done: E } = u.next();
        return E ? { value: p, done: E } : {
          value: l ? [f(p[0]), f(p[1])] : f(p),
          done: E
        };
      },
      [Symbol.iterator]() {
        return this;
      }
    };
  };
}
function WA(A) {
  return function(...e) {
    if (process.env.NODE_ENV !== "production") {
      const t = e[0] ? `on key "${e[0]}" ` : "";
      console.warn(`${Nt(A)} operation ${t}failed: target is readonly.`, U(this));
    }
    return A === "delete" ? !1 : this;
  };
}
function li() {
  const A = {
    get(s) {
      return Pe(this, s);
    },
    get size() {
      return $e(this);
    },
    has: _e,
    add: qo,
    set: Wo,
    delete: Lo,
    clear: zo,
    forEach: At(!1, !1)
  }, e = {
    get(s) {
      return Pe(this, s, !1, !0);
    },
    get size() {
      return $e(this);
    },
    has: _e,
    add: qo,
    set: Wo,
    delete: Lo,
    clear: zo,
    forEach: At(!1, !0)
  }, t = {
    get(s) {
      return Pe(this, s, !0);
    },
    get size() {
      return $e(this, !0);
    },
    has(s) {
      return _e.call(this, s, !0);
    },
    add: WA("add"),
    set: WA("set"),
    delete: WA("delete"),
    clear: WA("clear"),
    forEach: At(!0, !1)
  }, o = {
    get(s) {
      return Pe(this, s, !0, !0);
    },
    get size() {
      return $e(this, !0);
    },
    has(s) {
      return _e.call(this, s, !0);
    },
    add: WA("add"),
    set: WA("set"),
    delete: WA("delete"),
    clear: WA("clear"),
    forEach: At(!0, !0)
  };
  return ["keys", "values", "entries", Symbol.iterator].forEach((s) => {
    A[s] = et(s, !1, !1), t[s] = et(s, !0, !1), e[s] = et(s, !1, !0), o[s] = et(s, !0, !0);
  }), [
    A,
    t,
    e,
    o
  ];
}
const [ci, ai, ui, pi] = /* @__PURE__ */ li();
function Bt(A, e) {
  const t = e ? A ? pi : ui : A ? ai : ci;
  return (o, n, s) => n === "__v_isReactive" ? !A : n === "__v_isReadonly" ? A : n === "__v_raw" ? o : Reflect.get(V(t, n) && n in o ? t : o, n, s);
}
const fi = {
  get: /* @__PURE__ */ Bt(!1, !1)
}, hi = {
  get: /* @__PURE__ */ Bt(!1, !0)
}, di = {
  get: /* @__PURE__ */ Bt(!0, !1)
}, gi = {
  get: /* @__PURE__ */ Bt(!0, !0)
};
function Dn(A, e, t) {
  const o = U(t);
  if (o !== t && e.call(A, o)) {
    const n = mo(A);
    console.warn(`Reactive ${n} contains both the raw and reactive versions of the same object${n === "Map" ? " as keys" : ""}, which can lead to inconsistencies. Avoid differentiating between the raw and reactive versions of an object and only use the reactive version if possible.`);
  }
}
const Un = /* @__PURE__ */ new WeakMap(), Kn = /* @__PURE__ */ new WeakMap(), kn = /* @__PURE__ */ new WeakMap(), Vn = /* @__PURE__ */ new WeakMap();
function Ei(A) {
  switch (A) {
    case "Object":
    case "Array":
      return 1;
    case "Map":
    case "Set":
    case "WeakMap":
    case "WeakSet":
      return 2;
    default:
      return 0;
  }
}
function mi(A) {
  return A.__v_skip || !Object.isExtensible(A) ? 0 : Ei(mo(A));
}
function xo(A) {
  return PA(A) ? A : yt(A, !1, On, fi, Un);
}
function Mi(A) {
  return yt(A, !1, ii, hi, Kn);
}
function Jn(A) {
  return yt(A, !0, Rn, di, kn);
}
function Ie(A) {
  return yt(A, !0, ri, gi, Vn);
}
function yt(A, e, t, o, n) {
  if (!b(A))
    return process.env.NODE_ENV !== "production" && console.warn(`value cannot be made reactive: ${String(A)}`), A;
  if (A.__v_raw && !(e && A.__v_isReactive))
    return A;
  const s = n.get(A);
  if (s)
    return s;
  const i = mi(A);
  if (i === 0)
    return A;
  const l = new Proxy(A, i === 2 ? o : t);
  return n.set(A, l), l;
}
function ie(A) {
  return PA(A) ? ie(A.__v_raw) : !!(A && A.__v_isReactive);
}
function PA(A) {
  return !!(A && A.__v_isReadonly);
}
function dt(A) {
  return !!(A && A.__v_isShallow);
}
function _t(A) {
  return ie(A) || PA(A);
}
function U(A) {
  const e = A && A.__v_raw;
  return e ? U(e) : A;
}
function Tn(A) {
  return ft(A, "__v_skip", !0), A;
}
const Te = (A) => b(A) ? xo(A) : A, No = (A) => b(A) ? Jn(A) : A;
function jn(A) {
  vA && uA && (A = U(A), process.env.NODE_ENV !== "production" ? Pt(A.dep || (A.dep = Je()), {
    target: A,
    type: "get",
    key: "value"
  }) : Pt(A.dep || (A.dep = Je())));
}
function Zn(A, e) {
  A = U(A), A.dep && (process.env.NODE_ENV !== "production" ? Ce(A.dep, {
    target: A,
    type: "set",
    key: "value",
    newValue: e
  }) : Ce(A.dep));
}
function nA(A) {
  return !!(A && A.__v_isRef === !0);
}
function Oe(A) {
  return Ci(A, !1);
}
function Ci(A, e) {
  return nA(A) ? A : new Ii(A, e);
}
class Ii {
  constructor(e, t) {
    this.__v_isShallow = t, this.dep = void 0, this.__v_isRef = !0, this._rawValue = t ? e : U(e), this._value = t ? e : Te(e);
  }
  get value() {
    return jn(this), this._value;
  }
  set value(e) {
    const t = this.__v_isShallow || dt(e) || PA(e);
    e = t ? e : U(e), Ve(e, this._rawValue) && (this._rawValue = e, this._value = t ? e : Te(e), Zn(this, e));
  }
}
function S(A) {
  return nA(A) ? A.value : A;
}
const xi = {
  get: (A, e, t) => S(Reflect.get(A, e, t)),
  set: (A, e, t, o) => {
    const n = A[e];
    return nA(n) && !nA(t) ? (n.value = t, !0) : Reflect.set(A, e, t, o);
  }
};
function Fn(A) {
  return ie(A) ? A : new Proxy(A, xi);
}
var bn;
class Ni {
  constructor(e, t, o, n) {
    this._setter = t, this.dep = void 0, this.__v_isRef = !0, this[bn] = !1, this._dirty = !0, this.effect = new Co(e, () => {
      this._dirty || (this._dirty = !0, Zn(this));
    }), this.effect.computed = this, this.effect.active = this._cacheable = !n, this.__v_isReadonly = o;
  }
  get value() {
    const e = U(this);
    return jn(e), (e._dirty || !e._cacheable) && (e._dirty = !1, e._value = e.effect.run()), e._value;
  }
  set value(e) {
    this._setter(e);
  }
}
bn = "__v_isReadonly";
function wi(A, e, t = !1) {
  let o, n;
  const s = R(A);
  s ? (o = A, n = process.env.NODE_ENV !== "production" ? () => {
    console.warn("Write operation failed: computed value is readonly");
  } : iA) : (o = A.get, n = A.set);
  const i = new Ni(o, n, s || !n, t);
  return process.env.NODE_ENV !== "production" && e && !t && (i.effect.onTrack = e.onTrack, i.effect.onTrigger = e.onTrigger), i;
}
const re = [];
function st(A) {
  re.push(A);
}
function it() {
  re.pop();
}
function x(A, ...e) {
  if (process.env.NODE_ENV === "production")
    return;
  fe();
  const t = re.length ? re[re.length - 1].component : null, o = t && t.appContext.config.warnHandler, n = Gi();
  if (o)
    jA(o, t, 11, [
      A + e.join(""),
      t && t.proxy,
      n.map(({ vnode: s }) => `at <${Ut(t, s.type)}>`).join(`
`),
      n
    ]);
  else {
    const s = [`[Vue warn]: ${A}`, ...e];
    n.length && s.push(`
`, ...Bi(n)), console.warn(...s);
  }
  he();
}
function Gi() {
  let A = re[re.length - 1];
  if (!A)
    return [];
  const e = [];
  for (; A; ) {
    const t = e[0];
    t && t.vnode === A ? t.recurseCount++ : e.push({
      vnode: A,
      recurseCount: 0
    });
    const o = A.component && A.component.parent;
    A = o && o.vnode;
  }
  return e;
}
function Bi(A) {
  const e = [];
  return A.forEach((t, o) => {
    e.push(...o === 0 ? [] : [`
`], ...yi(t));
  }), e;
}
function yi({ vnode: A, recurseCount: e }) {
  const t = e > 0 ? `... (${e} recursive calls)` : "", o = A.component ? A.component.parent == null : !1, n = ` at <${Ut(A.component, A.type, o)}`, s = ">" + t;
  return A.props ? [n, ...Yi(A.props), s] : [n + s];
}
function Yi(A) {
  const e = [], t = Object.keys(A);
  return t.slice(0, 3).forEach((o) => {
    e.push(...qn(o, A[o]));
  }), t.length > 3 && e.push(" ..."), e;
}
function qn(A, e, t) {
  return AA(e) ? (e = JSON.stringify(e), t ? e : [`${A}=${e}`]) : typeof e == "number" || typeof e == "boolean" || e == null ? t ? e : [`${A}=${e}`] : nA(e) ? (e = qn(A, U(e.value), !0), t ? e : [`${A}=Ref<`, e, ">"]) : R(e) ? [`${A}=fn${e.name ? `<${e.name}>` : ""}`] : (e = U(e), t ? e : [`${A}=`, e]);
}
const wo = {
  sp: "serverPrefetch hook",
  bc: "beforeCreate hook",
  c: "created hook",
  bm: "beforeMount hook",
  m: "mounted hook",
  bu: "beforeUpdate hook",
  u: "updated",
  bum: "beforeUnmount hook",
  um: "unmounted hook",
  a: "activated hook",
  da: "deactivated hook",
  ec: "errorCaptured hook",
  rtc: "renderTracked hook",
  rtg: "renderTriggered hook",
  [0]: "setup function",
  [1]: "render function",
  [2]: "watcher getter",
  [3]: "watcher callback",
  [4]: "watcher cleanup function",
  [5]: "native event handler",
  [6]: "component event handler",
  [7]: "vnode hook",
  [8]: "directive hook",
  [9]: "transition hook",
  [10]: "app errorHandler",
  [11]: "app warnHandler",
  [12]: "ref function",
  [13]: "async component loader",
  [14]: "scheduler flush. This is likely a Vue internals bug. Please open an issue at https://new-issue.vuejs.org/?repo=vuejs/core"
};
function jA(A, e, t, o) {
  let n;
  try {
    n = o ? A(...o) : A();
  } catch (s) {
    Yt(s, e, t);
  }
  return n;
}
function CA(A, e, t, o) {
  if (R(A)) {
    const s = jA(A, e, t, o);
    return s && Eo(s) && s.catch((i) => {
      Yt(i, e, t);
    }), s;
  }
  const n = [];
  for (let s = 0; s < A.length; s++)
    n.push(CA(A[s], e, t, o));
  return n;
}
function Yt(A, e, t, o = !0) {
  const n = e ? e.vnode : null;
  if (e) {
    let s = e.parent;
    const i = e.proxy, l = process.env.NODE_ENV !== "production" ? wo[t] : t;
    for (; s; ) {
      const u = s.ec;
      if (u) {
        for (let f = 0; f < u.length; f++)
          if (u[f](A, i, l) === !1)
            return;
      }
      s = s.parent;
    }
    const c = e.appContext.config.errorHandler;
    if (c) {
      jA(c, null, 10, [A, i, l]);
      return;
    }
  }
  Qi(A, t, n, o);
}
function Qi(A, e, t, o = !0) {
  if (process.env.NODE_ENV !== "production") {
    const n = wo[e];
    if (t && st(t), x(`Unhandled error${n ? ` during execution of ${n}` : ""}`), t && it(), o)
      throw A;
    console.error(A);
  } else
    console.error(A);
}
let je = !1, $t = !1;
const lA = [];
let RA = 0;
const we = [];
let OA = null, LA = 0;
const Wn = /* @__PURE__ */ Promise.resolve();
let Go = null;
const Si = 100;
function Ln(A) {
  const e = Go || Wn;
  return A ? e.then(this ? A.bind(this) : A) : e;
}
function Oi(A) {
  let e = RA + 1, t = lA.length;
  for (; e < t; ) {
    const o = e + t >>> 1;
    Ze(lA[o]) < A ? e = o + 1 : t = o;
  }
  return e;
}
function Qt(A) {
  (!lA.length || !lA.includes(A, je && A.allowRecurse ? RA + 1 : RA)) && (A.id == null ? lA.push(A) : lA.splice(Oi(A.id), 0, A), zn());
}
function zn() {
  !je && !$t && ($t = !0, Go = Wn.then(Xn));
}
function Ri(A) {
  const e = lA.indexOf(A);
  e > RA && lA.splice(e, 1);
}
function Hn(A) {
  Q(A) ? we.push(...A) : (!OA || !OA.includes(A, A.allowRecurse ? LA + 1 : LA)) && we.push(A), zn();
}
function Ho(A, e = je ? RA + 1 : 0) {
  for (process.env.NODE_ENV !== "production" && (A = A || /* @__PURE__ */ new Map()); e < lA.length; e++) {
    const t = lA[e];
    if (t && t.pre) {
      if (process.env.NODE_ENV !== "production" && Bo(A, t))
        continue;
      lA.splice(e, 1), e--, t();
    }
  }
}
function vn(A) {
  if (we.length) {
    const e = [...new Set(we)];
    if (we.length = 0, OA) {
      OA.push(...e);
      return;
    }
    for (OA = e, process.env.NODE_ENV !== "production" && (A = A || /* @__PURE__ */ new Map()), OA.sort((t, o) => Ze(t) - Ze(o)), LA = 0; LA < OA.length; LA++)
      process.env.NODE_ENV !== "production" && Bo(A, OA[LA]) || OA[LA]();
    OA = null, LA = 0;
  }
}
const Ze = (A) => A.id == null ? 1 / 0 : A.id, Di = (A, e) => {
  const t = Ze(A) - Ze(e);
  if (t === 0) {
    if (A.pre && !e.pre)
      return -1;
    if (e.pre && !A.pre)
      return 1;
  }
  return t;
};
function Xn(A) {
  $t = !1, je = !0, process.env.NODE_ENV !== "production" && (A = A || /* @__PURE__ */ new Map()), lA.sort(Di);
  const e = process.env.NODE_ENV !== "production" ? (t) => Bo(A, t) : iA;
  try {
    for (RA = 0; RA < lA.length; RA++) {
      const t = lA[RA];
      if (t && t.active !== !1) {
        if (process.env.NODE_ENV !== "production" && e(t))
          continue;
        jA(t, null, 14);
      }
    }
  } finally {
    RA = 0, lA.length = 0, vn(A), je = !1, Go = null, (lA.length || we.length) && Xn(A);
  }
}
function Bo(A, e) {
  if (!A.has(e))
    A.set(e, 1);
  else {
    const t = A.get(e);
    if (t > Si) {
      const o = e.ownerInstance, n = o && Ys(o.type);
      return x(`Maximum recursive updates exceeded${n ? ` in component <${n}>` : ""}. This means you have a reactive effect that is mutating its own dependencies and thus recursively triggering itself. Possible sources include component template, render function, updated hook or watcher source function.`), !0;
    } else
      A.set(e, t + 1);
  }
}
let le = !1;
const Me = /* @__PURE__ */ new Set();
process.env.NODE_ENV !== "production" && (Gn().__VUE_HMR_RUNTIME__ = {
  createRecord: Tt(Pn),
  rerender: Tt(ki),
  reload: Tt(Vi)
});
const pe = /* @__PURE__ */ new Map();
function Ui(A) {
  const e = A.type.__hmrId;
  let t = pe.get(e);
  t || (Pn(e, A.type), t = pe.get(e)), t.instances.add(A);
}
function Ki(A) {
  pe.get(A.type.__hmrId).instances.delete(A);
}
function Pn(A, e) {
  return pe.has(A) ? !1 : (pe.set(A, {
    initialDef: Ke(e),
    instances: /* @__PURE__ */ new Set()
  }), !0);
}
function Ke(A) {
  return Qs(A) ? A.__vccOpts : A;
}
function ki(A, e) {
  const t = pe.get(A);
  t && (t.initialDef.render = e, [...t.instances].forEach((o) => {
    e && (o.render = e, Ke(o.type).render = e), o.renderCache = [], le = !0, o.update(), le = !1;
  }));
}
function Vi(A, e) {
  const t = pe.get(A);
  if (!t)
    return;
  e = Ke(e), vo(t.initialDef, e);
  const o = [...t.instances];
  for (const n of o) {
    const s = Ke(n.type);
    Me.has(s) || (s !== t.initialDef && vo(s, e), Me.add(s)), n.appContext.optionsCache.delete(n.type), n.ceReload ? (Me.add(s), n.ceReload(e.styles), Me.delete(s)) : n.parent ? Qt(n.parent.update) : n.appContext.reload ? n.appContext.reload() : typeof window < "u" ? window.location.reload() : console.warn("[HMR] Root or manually mounted instance modified. Full reload required.");
  }
  Hn(() => {
    for (const n of o)
      Me.delete(Ke(n.type));
  });
}
function vo(A, e) {
  _(A, e);
  for (const t in A)
    t !== "__file" && !(t in e) && delete A[t];
}
function Tt(A) {
  return (e, t) => {
    try {
      return A(e, t);
    } catch (o) {
      console.error(o), console.warn("[HMR] Something went wrong during Vue component hot-reload. Full reload required.");
    }
  };
}
let DA, Ue = [], Ao = !1;
function Le(A, ...e) {
  DA ? DA.emit(A, ...e) : Ao || Ue.push({ event: A, args: e });
}
function _n(A, e) {
  var t, o;
  DA = A, DA ? (DA.enabled = !0, Ue.forEach(({ event: n, args: s }) => DA.emit(n, ...s)), Ue = []) : typeof window < "u" && window.HTMLElement && !(!((o = (t = window.navigator) === null || t === void 0 ? void 0 : t.userAgent) === null || o === void 0) && o.includes("jsdom")) ? ((e.__VUE_DEVTOOLS_HOOK_REPLAY__ = e.__VUE_DEVTOOLS_HOOK_REPLAY__ || []).push((s) => {
    _n(s, e);
  }), setTimeout(() => {
    DA || (e.__VUE_DEVTOOLS_HOOK_REPLAY__ = null, Ao = !0, Ue = []);
  }, 3e3)) : (Ao = !0, Ue = []);
}
function Ji(A, e) {
  Le("app:init", A, e, {
    Fragment: gA,
    Text: He,
    Comment: pA,
    Static: at
  });
}
function Ti(A) {
  Le("app:unmount", A);
}
const ji = /* @__PURE__ */ yo("component:added"), $n = /* @__PURE__ */ yo("component:updated"), Zi = /* @__PURE__ */ yo("component:removed"), Fi = (A) => {
  DA && typeof DA.cleanupBuffer == "function" && !DA.cleanupBuffer(A) && Zi(A);
};
function yo(A) {
  return (e) => {
    Le(A, e.appContext.app, e.uid, e.parent ? e.parent.uid : void 0, e);
  };
}
const bi = /* @__PURE__ */ As("perf:start"), qi = /* @__PURE__ */ As("perf:end");
function As(A) {
  return (e, t, o) => {
    Le(A, e.appContext.app, e.uid, e, t, o);
  };
}
function Wi(A, e, t) {
  Le("component:emit", A.appContext.app, A, e, t);
}
function Li(A, e, ...t) {
  if (A.isUnmounted)
    return;
  const o = A.vnode.props || L;
  if (process.env.NODE_ENV !== "production") {
    const { emitsOptions: f, propsOptions: [p] } = A;
    if (f)
      if (!(e in f))
        (!p || !(Ae(e) in p)) && x(`Component emitted event "${e}" but it is neither declared in the emits option nor as an "${Ae(e)}" prop.`);
      else {
        const E = f[e];
        R(E) && (E(...t) || x(`Invalid event arguments: event validation failed for event "${e}".`));
      }
  }
  let n = t;
  const s = e.startsWith("update:"), i = s && e.slice(7);
  if (i && i in o) {
    const f = `${i === "modelValue" ? "model" : i}Modifiers`, { number: p, trim: E } = o[f] || L;
    E && (n = t.map((G) => AA(G) ? G.trim() : G)), p && (n = t.map(ht));
  }
  if (process.env.NODE_ENV !== "production" && Wi(A, e, n), process.env.NODE_ENV !== "production") {
    const f = e.toLowerCase();
    f !== e && o[Ae(f)] && x(`Event "${f}" is emitted in component ${Ut(A, A.type)} but the handler is registered for "${e}". Note that HTML attributes are case-insensitive and you cannot use v-on to listen to camelCase events when using in-DOM templates. You should probably use "${EA(e)}" instead of "${e}".`);
  }
  let l, c = o[l = Ae(e)] || o[l = Ae(TA(e))];
  !c && s && (c = o[l = Ae(EA(e))]), c && CA(c, A, 6, n);
  const u = o[l + "Once"];
  if (u) {
    if (!A.emitted)
      A.emitted = {};
    else if (A.emitted[l])
      return;
    A.emitted[l] = !0, CA(u, A, 6, n);
  }
}
function es(A, e, t = !1) {
  const o = e.emitsCache, n = o.get(A);
  if (n !== void 0)
    return n;
  const s = A.emits;
  let i = {}, l = !1;
  if (!R(A)) {
    const c = (u) => {
      const f = es(u, e, !0);
      f && (l = !0, _(i, f));
    };
    !t && e.mixins.length && e.mixins.forEach(c), A.extends && c(A.extends), A.mixins && A.mixins.forEach(c);
  }
  return !s && !l ? (b(A) && o.set(A, null), null) : (Q(s) ? s.forEach((c) => i[c] = null) : _(i, s), b(A) && o.set(A, i), i);
}
function St(A, e) {
  return !A || !We(e) ? !1 : (e = e.slice(2).replace(/Once$/, ""), V(A, e[0].toLowerCase() + e.slice(1)) || V(A, EA(e)) || V(A, e));
}
let mA = null, Ot = null;
function gt(A) {
  const e = mA;
  return mA = A, Ot = A && A.type.__scopeId || null, e;
}
function zi(A) {
  Ot = A;
}
function Hi() {
  Ot = null;
}
function vi(A, e = mA, t) {
  if (!e || A._n)
    return A;
  const o = (...n) => {
    o._d && rn(-1);
    const s = gt(e);
    let i;
    try {
      i = A(...n);
    } finally {
      gt(s), o._d && rn(1);
    }
    return process.env.NODE_ENV !== "production" && $n(e), i;
  };
  return o._n = !0, o._c = !0, o._d = !0, o;
}
let eo = !1;
function Et() {
  eo = !0;
}
function jt(A) {
  const { type: e, vnode: t, proxy: o, withProxy: n, props: s, propsOptions: [i], slots: l, attrs: c, emit: u, render: f, renderCache: p, data: E, setupState: G, ctx: D, inheritAttrs: y } = A;
  let T, eA;
  const X = gt(A);
  process.env.NODE_ENV !== "production" && (eo = !1);
  try {
    if (t.shapeFlag & 4) {
      const q = n || o;
      T = xA(f.call(q, q, p, s, G, E, D)), eA = c;
    } else {
      const q = e;
      process.env.NODE_ENV !== "production" && c === s && Et(), T = xA(q.length > 1 ? q(s, process.env.NODE_ENV !== "production" ? {
        get attrs() {
          return Et(), c;
        },
        slots: l,
        emit: u
      } : { attrs: c, slots: l, emit: u }) : q(s, null)), eA = e.props ? c : Pi(c);
    }
  } catch (q) {
    ke.length = 0, Yt(q, A, 1), T = wA(pA);
  }
  let H = T, K;
  if (process.env.NODE_ENV !== "production" && T.patchFlag > 0 && T.patchFlag & 2048 && ([H, K] = Xi(T)), eA && y !== !1) {
    const q = Object.keys(eA), { shapeFlag: GA } = H;
    if (q.length) {
      if (GA & 7)
        i && q.some(pt) && (eA = _i(eA, i)), H = UA(H, eA);
      else if (process.env.NODE_ENV !== "production" && !eo && H.type !== pA) {
        const BA = Object.keys(c), k = [], z = [];
        for (let v = 0, rA = BA.length; v < rA; v++) {
          const sA = BA[v];
          We(sA) ? pt(sA) || k.push(sA[2].toLowerCase() + sA.slice(3)) : z.push(sA);
        }
        z.length && x(`Extraneous non-props attributes (${z.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text root nodes.`), k.length && x(`Extraneous non-emits event listeners (${k.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text root nodes. If the listener is intended to be a component custom event listener only, declare it using the "emits" option.`);
      }
    }
  }
  return t.dirs && (process.env.NODE_ENV !== "production" && !Xo(H) && x("Runtime directive used on component with non-element root node. The directives will not function as intended."), H = UA(H), H.dirs = H.dirs ? H.dirs.concat(t.dirs) : t.dirs), t.transition && (process.env.NODE_ENV !== "production" && !Xo(H) && x("Component inside <Transition> renders non-element root node that cannot be animated."), H.transition = t.transition), process.env.NODE_ENV !== "production" && K ? K(H) : T = H, gt(X), T;
}
const Xi = (A) => {
  const e = A.children, t = A.dynamicChildren, o = ts(e);
  if (!o)
    return [A, void 0];
  const n = e.indexOf(o), s = t ? t.indexOf(o) : -1, i = (l) => {
    e[n] = l, t && (s > -1 ? t[s] = l : l.patchFlag > 0 && (A.dynamicChildren = [...t, l]));
  };
  return [xA(o), i];
};
function ts(A) {
  let e;
  for (let t = 0; t < A.length; t++) {
    const o = A[t];
    if (Oo(o)) {
      if (o.type !== pA || o.children === "v-if") {
        if (e)
          return;
        e = o;
      }
    } else
      return;
  }
  return e;
}
const Pi = (A) => {
  let e;
  for (const t in A)
    (t === "class" || t === "style" || We(t)) && ((e || (e = {}))[t] = A[t]);
  return e;
}, _i = (A, e) => {
  const t = {};
  for (const o in A)
    (!pt(o) || !(o.slice(9) in e)) && (t[o] = A[o]);
  return t;
}, Xo = (A) => A.shapeFlag & 7 || A.type === pA;
function $i(A, e, t) {
  const { props: o, children: n, component: s } = A, { props: i, children: l, patchFlag: c } = e, u = s.emitsOptions;
  if (process.env.NODE_ENV !== "production" && (n || l) && le || e.dirs || e.transition)
    return !0;
  if (t && c >= 0) {
    if (c & 1024)
      return !0;
    if (c & 16)
      return o ? Po(o, i, u) : !!i;
    if (c & 8) {
      const f = e.dynamicProps;
      for (let p = 0; p < f.length; p++) {
        const E = f[p];
        if (i[E] !== o[E] && !St(u, E))
          return !0;
      }
    }
  } else
    return (n || l) && (!l || !l.$stable) ? !0 : o === i ? !1 : o ? i ? Po(o, i, u) : !0 : !!i;
  return !1;
}
function Po(A, e, t) {
  const o = Object.keys(e);
  if (o.length !== Object.keys(A).length)
    return !0;
  for (let n = 0; n < o.length; n++) {
    const s = o[n];
    if (e[s] !== A[s] && !St(t, s))
      return !0;
  }
  return !1;
}
function Ar({ vnode: A, parent: e }, t) {
  for (; e && e.subTree === A; )
    (A = e.vnode).el = t, e = e.parent;
}
const er = (A) => A.__isSuspense;
function tr(A, e) {
  e && e.pendingBranch ? Q(A) ? e.effects.push(...A) : e.effects.push(A) : Hn(A);
}
function or(A, e) {
  if (!oA)
    process.env.NODE_ENV !== "production" && x("provide() can only be used inside setup().");
  else {
    let t = oA.provides;
    const o = oA.parent && oA.parent.provides;
    o === t && (t = oA.provides = Object.create(o)), t[A] = e;
  }
}
function rt(A, e, t = !1) {
  const o = oA || mA;
  if (o) {
    const n = o.parent == null ? o.vnode.appContext && o.vnode.appContext.provides : o.parent.provides;
    if (n && A in n)
      return n[A];
    if (arguments.length > 1)
      return t && R(e) ? e.call(o.proxy) : e;
    process.env.NODE_ENV !== "production" && x(`injection "${String(A)}" not found.`);
  } else
    process.env.NODE_ENV !== "production" && x("inject() can only be used inside setup() or functional components.");
}
const tt = {};
function Zt(A, e, t) {
  return process.env.NODE_ENV !== "production" && !R(e) && x("`watch(fn, options?)` signature has been moved to a separate API. Use `watchEffect(fn, options?)` instead. `watch` now only supports `watch(source, cb, options?) signature."), os(A, e, t);
}
function os(A, e, { immediate: t, deep: o, flush: n, onTrack: s, onTrigger: i } = L) {
  process.env.NODE_ENV !== "production" && !e && (t !== void 0 && x('watch() "immediate" option is only respected when using the watch(source, callback, options?) signature.'), o !== void 0 && x('watch() "deep" option is only respected when using the watch(source, callback, options?) signature.'));
  const l = (K) => {
    x("Invalid watch source: ", K, "A watch source can only be a getter/effect function, a ref, a reactive object, or an array of these types.");
  }, c = oA;
  let u, f = !1, p = !1;
  if (nA(A) ? (u = () => A.value, f = dt(A)) : ie(A) ? (u = () => A, o = !0) : Q(A) ? (p = !0, f = A.some((K) => ie(K) || dt(K)), u = () => A.map((K) => {
    if (nA(K))
      return K.value;
    if (ie(K))
      return xe(K);
    if (R(K))
      return jA(K, c, 2);
    process.env.NODE_ENV !== "production" && l(K);
  })) : R(A) ? e ? u = () => jA(A, c, 2) : u = () => {
    if (!(c && c.isUnmounted))
      return E && E(), CA(A, c, 3, [G]);
  } : (u = iA, process.env.NODE_ENV !== "production" && l(A)), e && o) {
    const K = u;
    u = () => xe(K());
  }
  let E, G = (K) => {
    E = X.onStop = () => {
      jA(K, c, 4);
    };
  }, D;
  if (qe)
    if (G = iA, e ? t && CA(e, c, 3, [
      u(),
      p ? [] : void 0,
      G
    ]) : u(), n === "sync") {
      const K = ul();
      D = K.__watcherHandles || (K.__watcherHandles = []);
    } else
      return iA;
  let y = p ? new Array(A.length).fill(tt) : tt;
  const T = () => {
    if (X.active)
      if (e) {
        const K = X.run();
        (o || f || (p ? K.some((q, GA) => Ve(q, y[GA])) : Ve(K, y))) && (E && E(), CA(e, c, 3, [
          K,
          y === tt ? void 0 : p && y[0] === tt ? [] : y,
          G
        ]), y = K);
      } else
        X.run();
  };
  T.allowRecurse = !!e;
  let eA;
  n === "sync" ? eA = T : n === "post" ? eA = () => hA(T, c && c.suspense) : (T.pre = !0, c && (T.id = c.uid), eA = () => Qt(T));
  const X = new Co(u, eA);
  process.env.NODE_ENV !== "production" && (X.onTrack = s, X.onTrigger = i), e ? t ? T() : y = X.run() : n === "post" ? hA(X.run.bind(X), c && c.suspense) : X.run();
  const H = () => {
    X.stop(), c && c.scope && ho(c.scope.effects, X);
  };
  return D && D.push(H), H;
}
function nr(A, e, t) {
  const o = this.proxy, n = AA(A) ? A.includes(".") ? ns(o, A) : () => o[A] : A.bind(o, o);
  let s;
  R(e) ? s = e : (s = e.handler, t = e);
  const i = oA;
  Ge(this);
  const l = os(n, s.bind(o), t);
  return i ? Ge(i) : ue(), l;
}
function ns(A, e) {
  const t = e.split(".");
  return () => {
    let o = A;
    for (let n = 0; n < t.length && o; n++)
      o = o[t[n]];
    return o;
  };
}
function xe(A, e) {
  if (!b(A) || A.__v_skip || (e = e || /* @__PURE__ */ new Set(), e.has(A)))
    return A;
  if (e.add(A), nA(A))
    xe(A.value, e);
  else if (Q(A))
    for (let t = 0; t < A.length; t++)
      xe(A[t], e);
  else if (xn(A) || ne(A))
    A.forEach((t) => {
      xe(t, e);
    });
  else if (wn(A))
    for (const t in A)
      xe(A[t], e);
  return A;
}
function sr() {
  const A = {
    isMounted: !1,
    isLeaving: !1,
    isUnmounting: !1,
    leavingVNodes: /* @__PURE__ */ new Map()
  };
  return ls(() => {
    A.isMounted = !0;
  }), cs(() => {
    A.isUnmounting = !0;
  }), A;
}
const MA = [Function, Array], ir = {
  name: "BaseTransition",
  props: {
    mode: String,
    appear: Boolean,
    persisted: Boolean,
    onBeforeEnter: MA,
    onEnter: MA,
    onAfterEnter: MA,
    onEnterCancelled: MA,
    onBeforeLeave: MA,
    onLeave: MA,
    onAfterLeave: MA,
    onLeaveCancelled: MA,
    onBeforeAppear: MA,
    onAppear: MA,
    onAfterAppear: MA,
    onAppearCancelled: MA
  },
  setup(A, { slots: e }) {
    const t = el(), o = sr();
    let n;
    return () => {
      const s = e.default && is(e.default(), !0);
      if (!s || !s.length)
        return;
      let i = s[0];
      if (s.length > 1) {
        let y = !1;
        for (const T of s)
          if (T.type !== pA) {
            if (process.env.NODE_ENV !== "production" && y) {
              x("<transition> can only be used on a single element or component. Use <transition-group> for lists.");
              break;
            }
            if (i = T, y = !0, process.env.NODE_ENV === "production")
              break;
          }
      }
      const l = U(A), { mode: c } = l;
      if (process.env.NODE_ENV !== "production" && c && c !== "in-out" && c !== "out-in" && c !== "default" && x(`invalid <transition> mode: ${c}`), o.isLeaving)
        return Ft(i);
      const u = _o(i);
      if (!u)
        return Ft(i);
      const f = to(u, l, o, t);
      oo(u, f);
      const p = t.subTree, E = p && _o(p);
      let G = !1;
      const { getTransitionKey: D } = u.type;
      if (D) {
        const y = D();
        n === void 0 ? n = y : y !== n && (n = y, G = !0);
      }
      if (E && E.type !== pA && (!te(u, E) || G)) {
        const y = to(E, l, o, t);
        if (oo(E, y), c === "out-in")
          return o.isLeaving = !0, y.afterLeave = () => {
            o.isLeaving = !1, t.update.active !== !1 && t.update();
          }, Ft(i);
        c === "in-out" && u.type !== pA && (y.delayLeave = (T, eA, X) => {
          const H = ss(o, E);
          H[String(E.key)] = E, T._leaveCb = () => {
            eA(), T._leaveCb = void 0, delete f.delayedLeave;
          }, f.delayedLeave = X;
        });
      }
      return i;
    };
  }
}, rr = ir;
function ss(A, e) {
  const { leavingVNodes: t } = A;
  let o = t.get(e.type);
  return o || (o = /* @__PURE__ */ Object.create(null), t.set(e.type, o)), o;
}
function to(A, e, t, o) {
  const { appear: n, mode: s, persisted: i = !1, onBeforeEnter: l, onEnter: c, onAfterEnter: u, onEnterCancelled: f, onBeforeLeave: p, onLeave: E, onAfterLeave: G, onLeaveCancelled: D, onBeforeAppear: y, onAppear: T, onAfterAppear: eA, onAppearCancelled: X } = e, H = String(A.key), K = ss(t, A), q = (k, z) => {
    k && CA(k, o, 9, z);
  }, GA = (k, z) => {
    const v = z[1];
    q(k, z), Q(k) ? k.every((rA) => rA.length <= 1) && v() : k.length <= 1 && v();
  }, BA = {
    mode: s,
    persisted: i,
    beforeEnter(k) {
      let z = l;
      if (!t.isMounted)
        if (n)
          z = y || l;
        else
          return;
      k._leaveCb && k._leaveCb(!0);
      const v = K[H];
      v && te(A, v) && v.el._leaveCb && v.el._leaveCb(), q(z, [k]);
    },
    enter(k) {
      let z = c, v = u, rA = f;
      if (!t.isMounted)
        if (n)
          z = T || c, v = eA || u, rA = X || f;
        else
          return;
      let sA = !1;
      const KA = k._enterCb = (ve) => {
        sA || (sA = !0, ve ? q(rA, [k]) : q(v, [k]), BA.delayedLeave && BA.delayedLeave(), k._enterCb = void 0);
      };
      z ? GA(z, [k, KA]) : KA();
    },
    leave(k, z) {
      const v = String(A.key);
      if (k._enterCb && k._enterCb(!0), t.isUnmounting)
        return z();
      q(p, [k]);
      let rA = !1;
      const sA = k._leaveCb = (KA) => {
        rA || (rA = !0, z(), KA ? q(D, [k]) : q(G, [k]), k._leaveCb = void 0, K[v] === A && delete K[v]);
      };
      K[v] = A, E ? GA(E, [k, sA]) : sA();
    },
    clone(k) {
      return to(k, e, t, o);
    }
  };
  return BA;
}
function Ft(A) {
  if (ze(A))
    return A = UA(A), A.children = null, A;
}
function _o(A) {
  return ze(A) ? A.children ? A.children[0] : void 0 : A;
}
function oo(A, e) {
  A.shapeFlag & 6 && A.component ? oo(A.component.subTree, e) : A.shapeFlag & 128 ? (A.ssContent.transition = e.clone(A.ssContent), A.ssFallback.transition = e.clone(A.ssFallback)) : A.transition = e;
}
function is(A, e = !1, t) {
  let o = [], n = 0;
  for (let s = 0; s < A.length; s++) {
    let i = A[s];
    const l = t == null ? i.key : String(t) + String(i.key != null ? i.key : s);
    i.type === gA ? (i.patchFlag & 128 && n++, o = o.concat(is(i.children, e, l))) : (e || i.type !== pA) && o.push(l != null ? UA(i, { key: l }) : i);
  }
  if (n > 1)
    for (let s = 0; s < o.length; s++)
      o[s].patchFlag = -2;
  return o;
}
function ye(A) {
  return R(A) ? { setup: A, name: A.name } : A;
}
const lt = (A) => !!A.type.__asyncLoader, ze = (A) => A.type.__isKeepAlive;
function lr(A, e) {
  rs(A, "a", e);
}
function cr(A, e) {
  rs(A, "da", e);
}
function rs(A, e, t = oA) {
  const o = A.__wdc || (A.__wdc = () => {
    let n = t;
    for (; n; ) {
      if (n.isDeactivated)
        return;
      n = n.parent;
    }
    return A();
  });
  if (Rt(e, o, t), t) {
    let n = t.parent;
    for (; n && n.parent; )
      ze(n.parent.vnode) && ar(o, e, t, n), n = n.parent;
  }
}
function ar(A, e, t, o) {
  const n = Rt(e, A, o, !0);
  as(() => {
    ho(o[e], n);
  }, t);
}
function Rt(A, e, t = oA, o = !1) {
  if (t) {
    const n = t[A] || (t[A] = []), s = e.__weh || (e.__weh = (...i) => {
      if (t.isUnmounted)
        return;
      fe(), Ge(t);
      const l = CA(e, t, A, i);
      return ue(), he(), l;
    });
    return o ? n.unshift(s) : n.push(s), s;
  } else if (process.env.NODE_ENV !== "production") {
    const n = Ae(wo[A].replace(/ hook$/, ""));
    x(`${n} is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup(). If you are using async setup(), make sure to register lifecycle hooks before the first await statement.`);
  }
}
const FA = (A) => (e, t = oA) => (!qe || A === "sp") && Rt(A, (...o) => e(...o), t), ur = FA("bm"), ls = FA("m"), pr = FA("bu"), fr = FA("u"), cs = FA("bum"), as = FA("um"), hr = FA("sp"), dr = FA("rtg"), gr = FA("rtc");
function Er(A, e = oA) {
  Rt("ec", A, e);
}
function us(A) {
  Fs(A) && x("Do not use built-in directive ids as custom directive id: " + A);
}
function _A(A, e, t, o) {
  const n = A.dirs, s = e && e.dirs;
  for (let i = 0; i < n.length; i++) {
    const l = n[i];
    s && (l.oldValue = s[i].value);
    let c = l.dir[o];
    c && (fe(), CA(c, t, 8, [
      A.el,
      l,
      A,
      e
    ]), he());
  }
}
const mr = Symbol();
function Mr(A, e, t, o) {
  let n;
  const s = t && t[o];
  if (Q(A) || AA(A)) {
    n = new Array(A.length);
    for (let i = 0, l = A.length; i < l; i++)
      n[i] = e(A[i], i, void 0, s && s[i]);
  } else if (typeof A == "number") {
    process.env.NODE_ENV !== "production" && !Number.isInteger(A) && x(`The v-for range expect an integer value but got ${A}.`), n = new Array(A);
    for (let i = 0; i < A; i++)
      n[i] = e(i + 1, i, void 0, s && s[i]);
  } else if (b(A))
    if (A[Symbol.iterator])
      n = Array.from(A, (i, l) => e(i, l, void 0, s && s[l]));
    else {
      const i = Object.keys(A);
      n = new Array(i.length);
      for (let l = 0, c = i.length; l < c; l++) {
        const u = i[l];
        n[l] = e(A[u], u, l, s && s[l]);
      }
    }
  else
    n = [];
  return t && (t[o] = n), n;
}
const no = (A) => A ? Bs(A) ? Do(A) || A.proxy : no(A.parent) : null, ce = /* @__PURE__ */ _(/* @__PURE__ */ Object.create(null), {
  $: (A) => A,
  $el: (A) => A.vnode.el,
  $data: (A) => A.data,
  $props: (A) => process.env.NODE_ENV !== "production" ? Ie(A.props) : A.props,
  $attrs: (A) => process.env.NODE_ENV !== "production" ? Ie(A.attrs) : A.attrs,
  $slots: (A) => process.env.NODE_ENV !== "production" ? Ie(A.slots) : A.slots,
  $refs: (A) => process.env.NODE_ENV !== "production" ? Ie(A.refs) : A.refs,
  $parent: (A) => no(A.parent),
  $root: (A) => no(A.root),
  $emit: (A) => A.emit,
  $options: (A) => Qo(A),
  $forceUpdate: (A) => A.f || (A.f = () => Qt(A.update)),
  $nextTick: (A) => A.n || (A.n = Ln.bind(A.proxy)),
  $watch: (A) => nr.bind(A)
}), Yo = (A) => A === "_" || A === "$", bt = (A, e) => A !== L && !A.__isScriptSetup && V(A, e), ps = {
  get({ _: A }, e) {
    const { ctx: t, setupState: o, data: n, props: s, accessCache: i, type: l, appContext: c } = A;
    if (process.env.NODE_ENV !== "production" && e === "__isVue")
      return !0;
    let u;
    if (e[0] !== "$") {
      const G = i[e];
      if (G !== void 0)
        switch (G) {
          case 1:
            return o[e];
          case 2:
            return n[e];
          case 4:
            return t[e];
          case 3:
            return s[e];
        }
      else {
        if (bt(o, e))
          return i[e] = 1, o[e];
        if (n !== L && V(n, e))
          return i[e] = 2, n[e];
        if ((u = A.propsOptions[0]) && V(u, e))
          return i[e] = 3, s[e];
        if (t !== L && V(t, e))
          return i[e] = 4, t[e];
        so && (i[e] = 0);
      }
    }
    const f = ce[e];
    let p, E;
    if (f)
      return e === "$attrs" && (dA(A, "get", e), process.env.NODE_ENV !== "production" && Et()), f(A);
    if ((p = l.__cssModules) && (p = p[e]))
      return p;
    if (t !== L && V(t, e))
      return i[e] = 4, t[e];
    if (E = c.config.globalProperties, V(E, e))
      return E[e];
    process.env.NODE_ENV !== "production" && mA && (!AA(e) || e.indexOf("__v") !== 0) && (n !== L && Yo(e[0]) && V(n, e) ? x(`Property ${JSON.stringify(e)} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`) : A === mA && x(`Property ${JSON.stringify(e)} was accessed during render but is not defined on instance.`));
  },
  set({ _: A }, e, t) {
    const { data: o, setupState: n, ctx: s } = A;
    return bt(n, e) ? (n[e] = t, !0) : process.env.NODE_ENV !== "production" && n.__isScriptSetup && V(n, e) ? (x(`Cannot mutate <script setup> binding "${e}" from Options API.`), !1) : o !== L && V(o, e) ? (o[e] = t, !0) : V(A.props, e) ? (process.env.NODE_ENV !== "production" && x(`Attempting to mutate prop "${e}". Props are readonly.`), !1) : e[0] === "$" && e.slice(1) in A ? (process.env.NODE_ENV !== "production" && x(`Attempting to mutate public property "${e}". Properties starting with $ are reserved and readonly.`), !1) : (process.env.NODE_ENV !== "production" && e in A.appContext.config.globalProperties ? Object.defineProperty(s, e, {
      enumerable: !0,
      configurable: !0,
      value: t
    }) : s[e] = t, !0);
  },
  has({ _: { data: A, setupState: e, accessCache: t, ctx: o, appContext: n, propsOptions: s } }, i) {
    let l;
    return !!t[i] || A !== L && V(A, i) || bt(e, i) || (l = s[0]) && V(l, i) || V(o, i) || V(ce, i) || V(n.config.globalProperties, i);
  },
  defineProperty(A, e, t) {
    return t.get != null ? A._.accessCache[e] = 0 : V(t, "value") && this.set(A, e, t.value, null), Reflect.defineProperty(A, e, t);
  }
};
process.env.NODE_ENV !== "production" && (ps.ownKeys = (A) => (x("Avoid app logic that relies on enumerating keys on a component instance. The keys will be empty in production mode to avoid performance overhead."), Reflect.ownKeys(A)));
function Cr(A) {
  const e = {};
  return Object.defineProperty(e, "_", {
    configurable: !0,
    enumerable: !1,
    get: () => A
  }), Object.keys(ce).forEach((t) => {
    Object.defineProperty(e, t, {
      configurable: !0,
      enumerable: !1,
      get: () => ce[t](A),
      set: iA
    });
  }), e;
}
function Ir(A) {
  const { ctx: e, propsOptions: [t] } = A;
  t && Object.keys(t).forEach((o) => {
    Object.defineProperty(e, o, {
      enumerable: !0,
      configurable: !0,
      get: () => A.props[o],
      set: iA
    });
  });
}
function xr(A) {
  const { ctx: e, setupState: t } = A;
  Object.keys(U(t)).forEach((o) => {
    if (!t.__isScriptSetup) {
      if (Yo(o[0])) {
        x(`setup() return property ${JSON.stringify(o)} should not start with "$" or "_" which are reserved prefixes for Vue internals.`);
        return;
      }
      Object.defineProperty(e, o, {
        enumerable: !0,
        configurable: !0,
        get: () => t[o],
        set: iA
      });
    }
  });
}
function Nr() {
  const A = /* @__PURE__ */ Object.create(null);
  return (e, t) => {
    A[t] ? x(`${e} property "${t}" is already defined in ${A[t]}.`) : A[t] = e;
  };
}
let so = !0;
function wr(A) {
  const e = Qo(A), t = A.proxy, o = A.ctx;
  so = !1, e.beforeCreate && $o(e.beforeCreate, A, "bc");
  const {
    data: n,
    computed: s,
    methods: i,
    watch: l,
    provide: c,
    inject: u,
    created: f,
    beforeMount: p,
    mounted: E,
    beforeUpdate: G,
    updated: D,
    activated: y,
    deactivated: T,
    beforeDestroy: eA,
    beforeUnmount: X,
    destroyed: H,
    unmounted: K,
    render: q,
    renderTracked: GA,
    renderTriggered: BA,
    errorCaptured: k,
    serverPrefetch: z,
    expose: v,
    inheritAttrs: rA,
    components: sA,
    directives: KA,
    filters: ve
  } = e, bA = process.env.NODE_ENV !== "production" ? Nr() : null;
  if (process.env.NODE_ENV !== "production") {
    const [j] = A.propsOptions;
    if (j)
      for (const Z in j)
        bA("Props", Z);
  }
  if (u && Gr(u, o, bA, A.appContext.config.unwrapInjectedRef), i)
    for (const j in i) {
      const Z = i[j];
      R(Z) ? (process.env.NODE_ENV !== "production" ? Object.defineProperty(o, j, {
        value: Z.bind(t),
        configurable: !0,
        enumerable: !0,
        writable: !0
      }) : o[j] = Z.bind(t), process.env.NODE_ENV !== "production" && bA("Methods", j)) : process.env.NODE_ENV !== "production" && x(`Method "${j}" has type "${typeof Z}" in the component definition. Did you reference the function correctly?`);
    }
  if (n) {
    process.env.NODE_ENV !== "production" && !R(n) && x("The data option must be a function. Plain object usage is no longer supported.");
    const j = n.call(t, t);
    if (process.env.NODE_ENV !== "production" && Eo(j) && x("data() returned a Promise - note data() cannot be async; If you intend to perform data fetching before component renders, use async setup() + <Suspense>."), !b(j))
      process.env.NODE_ENV !== "production" && x("data() should return an object.");
    else if (A.data = xo(j), process.env.NODE_ENV !== "production")
      for (const Z in j)
        bA("Data", Z), Yo(Z[0]) || Object.defineProperty(o, Z, {
          configurable: !0,
          enumerable: !0,
          get: () => j[Z],
          set: iA
        });
  }
  if (so = !0, s)
    for (const j in s) {
      const Z = s[j], yA = R(Z) ? Z.bind(t, t) : R(Z.get) ? Z.get.bind(t, t) : iA;
      process.env.NODE_ENV !== "production" && yA === iA && x(`Computed property "${j}" has no getter.`);
      const Kt = !R(Z) && R(Z.set) ? Z.set.bind(t) : process.env.NODE_ENV !== "production" ? () => {
        x(`Write operation failed: computed property "${j}" is readonly.`);
      } : iA, Ye = cl({
        get: yA,
        set: Kt
      });
      Object.defineProperty(o, j, {
        enumerable: !0,
        configurable: !0,
        get: () => Ye.value,
        set: (de) => Ye.value = de
      }), process.env.NODE_ENV !== "production" && bA("Computed", j);
    }
  if (l)
    for (const j in l)
      fs(l[j], o, t, j);
  if (c) {
    const j = R(c) ? c.call(t) : c;
    Reflect.ownKeys(j).forEach((Z) => {
      or(Z, j[Z]);
    });
  }
  f && $o(f, A, "c");
  function fA(j, Z) {
    Q(Z) ? Z.forEach((yA) => j(yA.bind(t))) : Z && j(Z.bind(t));
  }
  if (fA(ur, p), fA(ls, E), fA(pr, G), fA(fr, D), fA(lr, y), fA(cr, T), fA(Er, k), fA(gr, GA), fA(dr, BA), fA(cs, X), fA(as, K), fA(hr, z), Q(v))
    if (v.length) {
      const j = A.exposed || (A.exposed = {});
      v.forEach((Z) => {
        Object.defineProperty(j, Z, {
          get: () => t[Z],
          set: (yA) => t[Z] = yA
        });
      });
    } else
      A.exposed || (A.exposed = {});
  q && A.render === iA && (A.render = q), rA != null && (A.inheritAttrs = rA), sA && (A.components = sA), KA && (A.directives = KA);
}
function Gr(A, e, t = iA, o = !1) {
  Q(A) && (A = io(A));
  for (const n in A) {
    const s = A[n];
    let i;
    b(s) ? "default" in s ? i = rt(s.from || n, s.default, !0) : i = rt(s.from || n) : i = rt(s), nA(i) ? o ? Object.defineProperty(e, n, {
      enumerable: !0,
      configurable: !0,
      get: () => i.value,
      set: (l) => i.value = l
    }) : (process.env.NODE_ENV !== "production" && x(`injected property "${n}" is a ref and will be auto-unwrapped and no longer needs \`.value\` in the next minor release. To opt-in to the new behavior now, set \`app.config.unwrapInjectedRef = true\` (this config is temporary and will not be needed in the future.)`), e[n] = i) : e[n] = i, process.env.NODE_ENV !== "production" && t("Inject", n);
  }
}
function $o(A, e, t) {
  CA(Q(A) ? A.map((o) => o.bind(e.proxy)) : A.bind(e.proxy), e, t);
}
function fs(A, e, t, o) {
  const n = o.includes(".") ? ns(t, o) : () => t[o];
  if (AA(A)) {
    const s = e[A];
    R(s) ? Zt(n, s) : process.env.NODE_ENV !== "production" && x(`Invalid watch handler specified by key "${A}"`, s);
  } else if (R(A))
    Zt(n, A.bind(t));
  else if (b(A))
    if (Q(A))
      A.forEach((s) => fs(s, e, t, o));
    else {
      const s = R(A.handler) ? A.handler.bind(t) : e[A.handler];
      R(s) ? Zt(n, s, A) : process.env.NODE_ENV !== "production" && x(`Invalid watch handler specified by key "${A.handler}"`, s);
    }
  else
    process.env.NODE_ENV !== "production" && x(`Invalid watch option: "${o}"`, A);
}
function Qo(A) {
  const e = A.type, { mixins: t, extends: o } = e, { mixins: n, optionsCache: s, config: { optionMergeStrategies: i } } = A.appContext, l = s.get(e);
  let c;
  return l ? c = l : !n.length && !t && !o ? c = e : (c = {}, n.length && n.forEach((u) => mt(c, u, i, !0)), mt(c, e, i)), b(e) && s.set(e, c), c;
}
function mt(A, e, t, o = !1) {
  const { mixins: n, extends: s } = e;
  s && mt(A, s, t, !0), n && n.forEach((i) => mt(A, i, t, !0));
  for (const i in e)
    if (o && i === "expose")
      process.env.NODE_ENV !== "production" && x('"expose" option is ignored when declared in mixins or extends. It should only be declared in the base component itself.');
    else {
      const l = Br[i] || t && t[i];
      A[i] = l ? l(A[i], e[i]) : e[i];
    }
  return A;
}
const Br = {
  data: An,
  props: ee,
  emits: ee,
  methods: ee,
  computed: ee,
  beforeCreate: aA,
  created: aA,
  beforeMount: aA,
  mounted: aA,
  beforeUpdate: aA,
  updated: aA,
  beforeDestroy: aA,
  beforeUnmount: aA,
  destroyed: aA,
  unmounted: aA,
  activated: aA,
  deactivated: aA,
  errorCaptured: aA,
  serverPrefetch: aA,
  components: ee,
  directives: ee,
  watch: Yr,
  provide: An,
  inject: yr
};
function An(A, e) {
  return e ? A ? function() {
    return _(R(A) ? A.call(this, this) : A, R(e) ? e.call(this, this) : e);
  } : e : A;
}
function yr(A, e) {
  return ee(io(A), io(e));
}
function io(A) {
  if (Q(A)) {
    const e = {};
    for (let t = 0; t < A.length; t++)
      e[A[t]] = A[t];
    return e;
  }
  return A;
}
function aA(A, e) {
  return A ? [...new Set([].concat(A, e))] : e;
}
function ee(A, e) {
  return A ? _(_(/* @__PURE__ */ Object.create(null), A), e) : e;
}
function Yr(A, e) {
  if (!A)
    return e;
  if (!e)
    return A;
  const t = _(/* @__PURE__ */ Object.create(null), A);
  for (const o in e)
    t[o] = aA(A[o], e[o]);
  return t;
}
function Qr(A, e, t, o = !1) {
  const n = {}, s = {};
  ft(s, Dt, 1), A.propsDefaults = /* @__PURE__ */ Object.create(null), hs(A, e, n, s);
  for (const i in A.propsOptions[0])
    i in n || (n[i] = void 0);
  process.env.NODE_ENV !== "production" && gs(e || {}, n, A), t ? A.props = o ? n : Mi(n) : A.type.props ? A.props = n : A.props = s, A.attrs = s;
}
function Sr(A) {
  for (; A; ) {
    if (A.type.__hmrId)
      return !0;
    A = A.parent;
  }
}
function Or(A, e, t, o) {
  const { props: n, attrs: s, vnode: { patchFlag: i } } = A, l = U(n), [c] = A.propsOptions;
  let u = !1;
  if (!(process.env.NODE_ENV !== "production" && Sr(A)) && (o || i > 0) && !(i & 16)) {
    if (i & 8) {
      const f = A.vnode.dynamicProps;
      for (let p = 0; p < f.length; p++) {
        let E = f[p];
        if (St(A.emitsOptions, E))
          continue;
        const G = e[E];
        if (c)
          if (V(s, E))
            G !== s[E] && (s[E] = G, u = !0);
          else {
            const D = TA(E);
            n[D] = ro(c, l, D, G, A, !1);
          }
        else
          G !== s[E] && (s[E] = G, u = !0);
      }
    }
  } else {
    hs(A, e, n, s) && (u = !0);
    let f;
    for (const p in l)
      (!e || !V(e, p) && ((f = EA(p)) === p || !V(e, f))) && (c ? t && (t[p] !== void 0 || t[f] !== void 0) && (n[p] = ro(c, l, p, void 0, A, !0)) : delete n[p]);
    if (s !== l)
      for (const p in s)
        (!e || !V(e, p)) && (delete s[p], u = !0);
  }
  u && ZA(A, "set", "$attrs"), process.env.NODE_ENV !== "production" && gs(e || {}, n, A);
}
function hs(A, e, t, o) {
  const [n, s] = A.propsOptions;
  let i = !1, l;
  if (e)
    for (let c in e) {
      if (nt(c))
        continue;
      const u = e[c];
      let f;
      n && V(n, f = TA(c)) ? !s || !s.includes(f) ? t[f] = u : (l || (l = {}))[f] = u : St(A.emitsOptions, c) || (!(c in o) || u !== o[c]) && (o[c] = u, i = !0);
    }
  if (s) {
    const c = U(t), u = l || L;
    for (let f = 0; f < s.length; f++) {
      const p = s[f];
      t[p] = ro(n, c, p, u[p], A, !V(u, p));
    }
  }
  return i;
}
function ro(A, e, t, o, n, s) {
  const i = A[t];
  if (i != null) {
    const l = V(i, "default");
    if (l && o === void 0) {
      const c = i.default;
      if (i.type !== Function && R(c)) {
        const { propsDefaults: u } = n;
        t in u ? o = u[t] : (Ge(n), o = u[t] = c.call(null, e), ue());
      } else
        o = c;
    }
    i[0] && (s && !l ? o = !1 : i[1] && (o === "" || o === EA(t)) && (o = !0));
  }
  return o;
}
function ds(A, e, t = !1) {
  const o = e.propsCache, n = o.get(A);
  if (n)
    return n;
  const s = A.props, i = {}, l = [];
  let c = !1;
  if (!R(A)) {
    const f = (p) => {
      c = !0;
      const [E, G] = ds(p, e, !0);
      _(i, E), G && l.push(...G);
    };
    !t && e.mixins.length && e.mixins.forEach(f), A.extends && f(A.extends), A.mixins && A.mixins.forEach(f);
  }
  if (!s && !c)
    return b(A) && o.set(A, Ne), Ne;
  if (Q(s))
    for (let f = 0; f < s.length; f++) {
      process.env.NODE_ENV !== "production" && !AA(s[f]) && x("props must be strings when using array syntax.", s[f]);
      const p = TA(s[f]);
      en(p) && (i[p] = L);
    }
  else if (s) {
    process.env.NODE_ENV !== "production" && !b(s) && x("invalid props options", s);
    for (const f in s) {
      const p = TA(f);
      if (en(p)) {
        const E = s[f], G = i[p] = Q(E) || R(E) ? { type: E } : Object.assign({}, E);
        if (G) {
          const D = on(Boolean, G.type), y = on(String, G.type);
          G[0] = D > -1, G[1] = y < 0 || D < y, (D > -1 || V(G, "default")) && l.push(p);
        }
      }
    }
  }
  const u = [i, l];
  return b(A) && o.set(A, u), u;
}
function en(A) {
  return A[0] !== "$" ? !0 : (process.env.NODE_ENV !== "production" && x(`Invalid prop name: "${A}" is a reserved property.`), !1);
}
function lo(A) {
  const e = A && A.toString().match(/^\s*function (\w+)/);
  return e ? e[1] : A === null ? "null" : "";
}
function tn(A, e) {
  return lo(A) === lo(e);
}
function on(A, e) {
  return Q(e) ? e.findIndex((t) => tn(t, A)) : R(e) && tn(e, A) ? 0 : -1;
}
function gs(A, e, t) {
  const o = U(e), n = t.propsOptions[0];
  for (const s in n) {
    let i = n[s];
    i != null && Rr(s, o[s], i, !V(A, s) && !V(A, EA(s)));
  }
}
function Rr(A, e, t, o) {
  const { type: n, required: s, validator: i } = t;
  if (s && o) {
    x('Missing required prop: "' + A + '"');
    return;
  }
  if (!(e == null && !t.required)) {
    if (n != null && n !== !0) {
      let l = !1;
      const c = Q(n) ? n : [n], u = [];
      for (let f = 0; f < c.length && !l; f++) {
        const { valid: p, expectedType: E } = Ur(e, c[f]);
        u.push(E || ""), l = p;
      }
      if (!l) {
        x(Kr(A, e, u));
        return;
      }
    }
    i && !i(e) && x('Invalid prop: custom validator check failed for prop "' + A + '".');
  }
}
const Dr = /* @__PURE__ */ Be("String,Number,Boolean,Function,Symbol,BigInt");
function Ur(A, e) {
  let t;
  const o = lo(e);
  if (Dr(o)) {
    const n = typeof A;
    t = n === o.toLowerCase(), !t && n === "object" && (t = A instanceof e);
  } else
    o === "Object" ? t = b(A) : o === "Array" ? t = Q(A) : o === "null" ? t = A === null : t = A instanceof e;
  return {
    valid: t,
    expectedType: o
  };
}
function Kr(A, e, t) {
  let o = `Invalid prop: type check failed for prop "${A}". Expected ${t.map(Nt).join(" | ")}`;
  const n = t[0], s = mo(e), i = nn(e, n), l = nn(e, s);
  return t.length === 1 && sn(n) && !kr(n, s) && (o += ` with value ${i}`), o += `, got ${s} `, sn(s) && (o += `with value ${l}.`), o;
}
function nn(A, e) {
  return e === "String" ? `"${A}"` : e === "Number" ? `${Number(A)}` : `${A}`;
}
function sn(A) {
  return ["string", "number", "boolean"].some((t) => A.toLowerCase() === t);
}
function kr(...A) {
  return A.some((e) => e.toLowerCase() === "boolean");
}
const Es = (A) => A[0] === "_" || A === "$stable", So = (A) => Q(A) ? A.map(xA) : [xA(A)], Vr = (A, e, t) => {
  if (e._n)
    return e;
  const o = vi((...n) => (process.env.NODE_ENV !== "production" && oA && x(`Slot "${A}" invoked outside of the render function: this will not track dependencies used in the slot. Invoke the slot function inside the render function instead.`), So(e(...n))), t);
  return o._c = !1, o;
}, ms = (A, e, t) => {
  const o = A._ctx;
  for (const n in A) {
    if (Es(n))
      continue;
    const s = A[n];
    if (R(s))
      e[n] = Vr(n, s, o);
    else if (s != null) {
      process.env.NODE_ENV !== "production" && x(`Non-function value encountered for slot "${n}". Prefer function slots for better performance.`);
      const i = So(s);
      e[n] = () => i;
    }
  }
}, Ms = (A, e) => {
  process.env.NODE_ENV !== "production" && !ze(A.vnode) && x("Non-function value encountered for default slot. Prefer function slots for better performance.");
  const t = So(e);
  A.slots.default = () => t;
}, Jr = (A, e) => {
  if (A.vnode.shapeFlag & 32) {
    const t = e._;
    t ? (A.slots = U(e), ft(e, "_", t)) : ms(e, A.slots = {});
  } else
    A.slots = {}, e && Ms(A, e);
  ft(A.slots, Dt, 1);
}, Tr = (A, e, t) => {
  const { vnode: o, slots: n } = A;
  let s = !0, i = L;
  if (o.shapeFlag & 32) {
    const l = e._;
    l ? process.env.NODE_ENV !== "production" && le ? _(n, e) : t && l === 1 ? s = !1 : (_(n, e), !t && l === 1 && delete n._) : (s = !e.$stable, ms(e, n)), i = e;
  } else
    e && (Ms(A, e), i = { default: 1 });
  if (s)
    for (const l in n)
      !Es(l) && !(l in i) && delete n[l];
};
function Cs() {
  return {
    app: null,
    config: {
      isNativeTag: In,
      performance: !1,
      globalProperties: {},
      optionMergeStrategies: {},
      errorHandler: void 0,
      warnHandler: void 0,
      compilerOptions: {}
    },
    mixins: [],
    components: {},
    directives: {},
    provides: /* @__PURE__ */ Object.create(null),
    optionsCache: /* @__PURE__ */ new WeakMap(),
    propsCache: /* @__PURE__ */ new WeakMap(),
    emitsCache: /* @__PURE__ */ new WeakMap()
  };
}
let jr = 0;
function Zr(A, e) {
  return function(o, n = null) {
    R(o) || (o = Object.assign({}, o)), n != null && !b(n) && (process.env.NODE_ENV !== "production" && x("root props passed to app.mount() must be an object."), n = null);
    const s = Cs(), i = /* @__PURE__ */ new Set();
    let l = !1;
    const c = s.app = {
      _uid: jr++,
      _component: o,
      _props: n,
      _container: null,
      _context: s,
      _instance: null,
      version: an,
      get config() {
        return s.config;
      },
      set config(u) {
        process.env.NODE_ENV !== "production" && x("app.config cannot be replaced. Modify individual options instead.");
      },
      use(u, ...f) {
        return i.has(u) ? process.env.NODE_ENV !== "production" && x("Plugin has already been applied to target app.") : u && R(u.install) ? (i.add(u), u.install(c, ...f)) : R(u) ? (i.add(u), u(c, ...f)) : process.env.NODE_ENV !== "production" && x('A plugin must either be a function or an object with an "install" function.'), c;
      },
      mixin(u) {
        return s.mixins.includes(u) ? process.env.NODE_ENV !== "production" && x("Mixin has already been applied to target app" + (u.name ? `: ${u.name}` : "")) : s.mixins.push(u), c;
      },
      component(u, f) {
        return process.env.NODE_ENV !== "production" && ao(u, s.config), f ? (process.env.NODE_ENV !== "production" && s.components[u] && x(`Component "${u}" has already been registered in target app.`), s.components[u] = f, c) : s.components[u];
      },
      directive(u, f) {
        return process.env.NODE_ENV !== "production" && us(u), f ? (process.env.NODE_ENV !== "production" && s.directives[u] && x(`Directive "${u}" has already been registered in target app.`), s.directives[u] = f, c) : s.directives[u];
      },
      mount(u, f, p) {
        if (l)
          process.env.NODE_ENV !== "production" && x("App has already been mounted.\nIf you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. `const createMyApp = () => createApp(App)`");
        else {
          process.env.NODE_ENV !== "production" && u.__vue_app__ && x("There is already an app instance mounted on the host container.\n If you want to mount another app on the same host container, you need to unmount the previous app by calling `app.unmount()` first.");
          const E = wA(o, n);
          return E.appContext = s, process.env.NODE_ENV !== "production" && (s.reload = () => {
            A(UA(E), u, p);
          }), f && e ? e(E, u) : A(E, u, p), l = !0, c._container = u, u.__vue_app__ = c, process.env.NODE_ENV !== "production" && (c._instance = E.component, Ji(c, an)), Do(E.component) || E.component.proxy;
        }
      },
      unmount() {
        l ? (A(null, c._container), process.env.NODE_ENV !== "production" && (c._instance = null, Ti(c)), delete c._container.__vue_app__) : process.env.NODE_ENV !== "production" && x("Cannot unmount an app that is not mounted.");
      },
      provide(u, f) {
        return process.env.NODE_ENV !== "production" && u in s.provides && x(`App already provides property with key "${String(u)}". It will be overwritten with the new value.`), s.provides[u] = f, c;
      }
    };
    return c;
  };
}
function co(A, e, t, o, n = !1) {
  if (Q(A)) {
    A.forEach((E, G) => co(E, e && (Q(e) ? e[G] : e), t, o, n));
    return;
  }
  if (lt(o) && !n)
    return;
  const s = o.shapeFlag & 4 ? Do(o.component) || o.component.proxy : o.el, i = n ? null : s, { i: l, r: c } = A;
  if (process.env.NODE_ENV !== "production" && !l) {
    x("Missing ref owner context. ref cannot be used on hoisted vnodes. A vnode with ref must be created inside the render function.");
    return;
  }
  const u = e && e.r, f = l.refs === L ? l.refs = {} : l.refs, p = l.setupState;
  if (u != null && u !== c && (AA(u) ? (f[u] = null, V(p, u) && (p[u] = null)) : nA(u) && (u.value = null)), R(c))
    jA(c, l, 12, [i, f]);
  else {
    const E = AA(c), G = nA(c);
    if (E || G) {
      const D = () => {
        if (A.f) {
          const y = E ? V(p, c) ? p[c] : f[c] : c.value;
          n ? Q(y) && ho(y, s) : Q(y) ? y.includes(s) || y.push(s) : E ? (f[c] = [s], V(p, c) && (p[c] = f[c])) : (c.value = [s], A.k && (f[A.k] = c.value));
        } else
          E ? (f[c] = i, V(p, c) && (p[c] = i)) : G ? (c.value = i, A.k && (f[A.k] = i)) : process.env.NODE_ENV !== "production" && x("Invalid template ref type:", c, `(${typeof c})`);
      };
      i ? (D.id = -1, hA(D, t)) : D();
    } else
      process.env.NODE_ENV !== "production" && x("Invalid template ref type:", c, `(${typeof c})`);
  }
}
let Re, HA;
function VA(A, e) {
  A.appContext.config.performance && Mt() && HA.mark(`vue-${e}-${A.uid}`), process.env.NODE_ENV !== "production" && bi(A, e, Mt() ? HA.now() : Date.now());
}
function JA(A, e) {
  if (A.appContext.config.performance && Mt()) {
    const t = `vue-${e}-${A.uid}`, o = t + ":end";
    HA.mark(o), HA.measure(`<${Ut(A, A.type)}> ${e}`, t, o), HA.clearMarks(t), HA.clearMarks(o);
  }
  process.env.NODE_ENV !== "production" && qi(A, e, Mt() ? HA.now() : Date.now());
}
function Mt() {
  return Re !== void 0 || (typeof window < "u" && window.performance ? (Re = !0, HA = window.performance) : Re = !1), Re;
}
function Fr() {
  const A = [];
  if (process.env.NODE_ENV !== "production" && A.length) {
    const e = A.length > 1;
    console.warn(`Feature flag${e ? "s" : ""} ${A.join(", ")} ${e ? "are" : "is"} not explicitly defined. You are running the esm-bundler build of Vue, which expects these compile-time feature flags to be globally injected via the bundler config in order to get better tree-shaking in the production bundle.

For more details, see https://link.vuejs.org/feature-flags.`);
  }
}
const hA = tr;
function br(A) {
  return qr(A);
}
function qr(A, e) {
  Fr();
  const t = Gn();
  t.__VUE__ = !0, process.env.NODE_ENV !== "production" && _n(t.__VUE_DEVTOOLS_GLOBAL_HOOK__, t);
  const { insert: o, remove: n, patchProp: s, createElement: i, createText: l, createComment: c, setText: u, setElementText: f, parentNode: p, nextSibling: E, setScopeId: G = iA, insertStaticContent: D } = A, y = (r, a, h, g = null, d = null, C = null, N = !1, M = null, I = process.env.NODE_ENV !== "production" && le ? !1 : !!a.dynamicChildren) => {
    if (r === a)
      return;
    r && !te(r, a) && (g = Xe(r), qA(r, d, C, !0), r = null), a.patchFlag === -2 && (I = !1, a.dynamicChildren = null);
    const { type: m, ref: B, shapeFlag: w } = a;
    switch (m) {
      case He:
        T(r, a, h, g);
        break;
      case pA:
        eA(r, a, h, g);
        break;
      case at:
        r == null ? X(a, h, g, N) : process.env.NODE_ENV !== "production" && H(r, a, h, N);
        break;
      case gA:
        KA(r, a, h, g, d, C, N, M, I);
        break;
      default:
        w & 1 ? GA(r, a, h, g, d, C, N, M, I) : w & 6 ? ve(r, a, h, g, d, C, N, M, I) : w & 64 || w & 128 ? m.process(r, a, h, g, d, C, N, M, I, ge) : process.env.NODE_ENV !== "production" && x("Invalid VNode type:", m, `(${typeof m})`);
    }
    B != null && d && co(B, r && r.ref, C, a || r, !a);
  }, T = (r, a, h, g) => {
    if (r == null)
      o(a.el = l(a.children), h, g);
    else {
      const d = a.el = r.el;
      a.children !== r.children && u(d, a.children);
    }
  }, eA = (r, a, h, g) => {
    r == null ? o(a.el = c(a.children || ""), h, g) : a.el = r.el;
  }, X = (r, a, h, g) => {
    [r.el, r.anchor] = D(r.children, a, h, g, r.el, r.anchor);
  }, H = (r, a, h, g) => {
    if (a.children !== r.children) {
      const d = E(r.anchor);
      q(r), [a.el, a.anchor] = D(a.children, h, d, g);
    } else
      a.el = r.el, a.anchor = r.anchor;
  }, K = ({ el: r, anchor: a }, h, g) => {
    let d;
    for (; r && r !== a; )
      d = E(r), o(r, h, g), r = d;
    o(a, h, g);
  }, q = ({ el: r, anchor: a }) => {
    let h;
    for (; r && r !== a; )
      h = E(r), n(r), r = h;
    n(a);
  }, GA = (r, a, h, g, d, C, N, M, I) => {
    N = N || a.type === "svg", r == null ? BA(a, h, g, d, C, N, M, I) : v(r, a, d, C, N, M, I);
  }, BA = (r, a, h, g, d, C, N, M) => {
    let I, m;
    const { type: B, props: w, shapeFlag: Y, transition: O, dirs: J } = r;
    if (I = r.el = i(r.type, C, w && w.is, w), Y & 8 ? f(I, r.children) : Y & 16 && z(r.children, I, null, g, d, C && B !== "foreignObject", N, M), J && _A(r, null, g, "created"), w) {
      for (const F in w)
        F !== "value" && !nt(F) && s(I, F, null, w[F], C, r.children, g, d, kA);
      "value" in w && s(I, "value", null, w.value), (m = w.onVnodeBeforeMount) && QA(m, g, r);
    }
    k(I, r, r.scopeId, N, g), process.env.NODE_ENV !== "production" && (Object.defineProperty(I, "__vnode", {
      value: r,
      enumerable: !1
    }), Object.defineProperty(I, "__vueParentComponent", {
      value: g,
      enumerable: !1
    })), J && _A(r, null, g, "beforeMount");
    const W = (!d || d && !d.pendingBranch) && O && !O.persisted;
    W && O.beforeEnter(I), o(I, a, h), ((m = w && w.onVnodeMounted) || W || J) && hA(() => {
      m && QA(m, g, r), W && O.enter(I), J && _A(r, null, g, "mounted");
    }, d);
  }, k = (r, a, h, g, d) => {
    if (h && G(r, h), g)
      for (let C = 0; C < g.length; C++)
        G(r, g[C]);
    if (d) {
      let C = d.subTree;
      if (process.env.NODE_ENV !== "production" && C.patchFlag > 0 && C.patchFlag & 2048 && (C = ts(C.children) || C), a === C) {
        const N = d.vnode;
        k(r, N, N.scopeId, N.slotScopeIds, d.parent);
      }
    }
  }, z = (r, a, h, g, d, C, N, M, I = 0) => {
    for (let m = I; m < r.length; m++) {
      const B = r[m] = M ? zA(r[m]) : xA(r[m]);
      y(null, B, a, h, g, d, C, N, M);
    }
  }, v = (r, a, h, g, d, C, N) => {
    const M = a.el = r.el;
    let { patchFlag: I, dynamicChildren: m, dirs: B } = a;
    I |= r.patchFlag & 16;
    const w = r.props || L, Y = a.props || L;
    let O;
    h && $A(h, !1), (O = Y.onVnodeBeforeUpdate) && QA(O, h, a, r), B && _A(a, r, h, "beforeUpdate"), h && $A(h, !0), process.env.NODE_ENV !== "production" && le && (I = 0, N = !1, m = null);
    const J = d && a.type !== "foreignObject";
    if (m ? (rA(r.dynamicChildren, m, M, h, g, J, C), process.env.NODE_ENV !== "production" && h && h.type.__hmrId && ct(r, a)) : N || yA(r, a, M, null, h, g, J, C, !1), I > 0) {
      if (I & 16)
        sA(M, a, w, Y, h, g, d);
      else if (I & 2 && w.class !== Y.class && s(M, "class", null, Y.class, d), I & 4 && s(M, "style", w.style, Y.style, d), I & 8) {
        const W = a.dynamicProps;
        for (let F = 0; F < W.length; F++) {
          const tA = W[F], IA = w[tA], Ee = Y[tA];
          (Ee !== IA || tA === "value") && s(M, tA, IA, Ee, d, r.children, h, g, kA);
        }
      }
      I & 1 && r.children !== a.children && f(M, a.children);
    } else
      !N && m == null && sA(M, a, w, Y, h, g, d);
    ((O = Y.onVnodeUpdated) || B) && hA(() => {
      O && QA(O, h, a, r), B && _A(a, r, h, "updated");
    }, g);
  }, rA = (r, a, h, g, d, C, N) => {
    for (let M = 0; M < a.length; M++) {
      const I = r[M], m = a[M], B = I.el && (I.type === gA || !te(I, m) || I.shapeFlag & 70) ? p(I.el) : h;
      y(I, m, B, null, g, d, C, N, !0);
    }
  }, sA = (r, a, h, g, d, C, N) => {
    if (h !== g) {
      if (h !== L)
        for (const M in h)
          !nt(M) && !(M in g) && s(r, M, h[M], null, N, a.children, d, C, kA);
      for (const M in g) {
        if (nt(M))
          continue;
        const I = g[M], m = h[M];
        I !== m && M !== "value" && s(r, M, m, I, N, a.children, d, C, kA);
      }
      "value" in g && s(r, "value", h.value, g.value);
    }
  }, KA = (r, a, h, g, d, C, N, M, I) => {
    const m = a.el = r ? r.el : l(""), B = a.anchor = r ? r.anchor : l("");
    let { patchFlag: w, dynamicChildren: Y, slotScopeIds: O } = a;
    process.env.NODE_ENV !== "production" && (le || w & 2048) && (w = 0, I = !1, Y = null), O && (M = M ? M.concat(O) : O), r == null ? (o(m, h, g), o(B, h, g), z(a.children, h, B, d, C, N, M, I)) : w > 0 && w & 64 && Y && r.dynamicChildren ? (rA(r.dynamicChildren, Y, h, d, C, N, M), process.env.NODE_ENV !== "production" && d && d.type.__hmrId ? ct(r, a) : (a.key != null || d && a === d.subTree) && ct(r, a, !0)) : yA(r, a, h, B, d, C, N, M, I);
  }, ve = (r, a, h, g, d, C, N, M, I) => {
    a.slotScopeIds = M, r == null ? a.shapeFlag & 512 ? d.ctx.activate(a, h, g, N, I) : bA(a, h, g, d, C, N, I) : fA(r, a, I);
  }, bA = (r, a, h, g, d, C, N) => {
    const M = r.component = Al(r, g, d);
    if (process.env.NODE_ENV !== "production" && M.type.__hmrId && Ui(M), process.env.NODE_ENV !== "production" && (st(r), VA(M, "mount")), ze(r) && (M.ctx.renderer = ge), process.env.NODE_ENV !== "production" && VA(M, "init"), ol(M), process.env.NODE_ENV !== "production" && JA(M, "init"), M.asyncDep) {
      if (d && d.registerDep(M, j), !r.el) {
        const I = M.subTree = wA(pA);
        eA(null, I, a, h);
      }
      return;
    }
    j(M, r, a, h, d, C, N), process.env.NODE_ENV !== "production" && (it(), JA(M, "mount"));
  }, fA = (r, a, h) => {
    const g = a.component = r.component;
    if ($i(r, a, h))
      if (g.asyncDep && !g.asyncResolved) {
        process.env.NODE_ENV !== "production" && st(a), Z(g, a, h), process.env.NODE_ENV !== "production" && it();
        return;
      } else
        g.next = a, Ri(g.update), g.update();
    else
      a.el = r.el, g.vnode = a;
  }, j = (r, a, h, g, d, C, N) => {
    const M = () => {
      if (r.isMounted) {
        let { next: B, bu: w, u: Y, parent: O, vnode: J } = r, W = B, F;
        process.env.NODE_ENV !== "production" && st(B || r.vnode), $A(r, !1), B ? (B.el = J.el, Z(r, B, N)) : B = J, w && Se(w), (F = B.props && B.props.onVnodeBeforeUpdate) && QA(F, O, B, J), $A(r, !0), process.env.NODE_ENV !== "production" && VA(r, "render");
        const tA = jt(r);
        process.env.NODE_ENV !== "production" && JA(r, "render");
        const IA = r.subTree;
        r.subTree = tA, process.env.NODE_ENV !== "production" && VA(r, "patch"), y(
          IA,
          tA,
          p(IA.el),
          Xe(IA),
          r,
          d,
          C
        ), process.env.NODE_ENV !== "production" && JA(r, "patch"), B.el = tA.el, W === null && Ar(r, tA.el), Y && hA(Y, d), (F = B.props && B.props.onVnodeUpdated) && hA(() => QA(F, O, B, J), d), process.env.NODE_ENV !== "production" && $n(r), process.env.NODE_ENV !== "production" && it();
      } else {
        let B;
        const { el: w, props: Y } = a, { bm: O, m: J, parent: W } = r, F = lt(a);
        if ($A(r, !1), O && Se(O), !F && (B = Y && Y.onVnodeBeforeMount) && QA(B, W, a), $A(r, !0), w && Jt) {
          const tA = () => {
            process.env.NODE_ENV !== "production" && VA(r, "render"), r.subTree = jt(r), process.env.NODE_ENV !== "production" && JA(r, "render"), process.env.NODE_ENV !== "production" && VA(r, "hydrate"), Jt(w, r.subTree, r, d, null), process.env.NODE_ENV !== "production" && JA(r, "hydrate");
          };
          F ? a.type.__asyncLoader().then(
            () => !r.isUnmounted && tA()
          ) : tA();
        } else {
          process.env.NODE_ENV !== "production" && VA(r, "render");
          const tA = r.subTree = jt(r);
          process.env.NODE_ENV !== "production" && JA(r, "render"), process.env.NODE_ENV !== "production" && VA(r, "patch"), y(null, tA, h, g, r, d, C), process.env.NODE_ENV !== "production" && JA(r, "patch"), a.el = tA.el;
        }
        if (J && hA(J, d), !F && (B = Y && Y.onVnodeMounted)) {
          const tA = a;
          hA(() => QA(B, W, tA), d);
        }
        (a.shapeFlag & 256 || W && lt(W.vnode) && W.vnode.shapeFlag & 256) && r.a && hA(r.a, d), r.isMounted = !0, process.env.NODE_ENV !== "production" && ji(r), a = h = g = null;
      }
    }, I = r.effect = new Co(
      M,
      () => Qt(m),
      r.scope
    ), m = r.update = () => I.run();
    m.id = r.uid, $A(r, !0), process.env.NODE_ENV !== "production" && (I.onTrack = r.rtc ? (B) => Se(r.rtc, B) : void 0, I.onTrigger = r.rtg ? (B) => Se(r.rtg, B) : void 0, m.ownerInstance = r), m();
  }, Z = (r, a, h) => {
    a.component = r;
    const g = r.vnode.props;
    r.vnode = a, r.next = null, Or(r, a.props, g, h), Tr(r, a.children, h), fe(), Ho(), he();
  }, yA = (r, a, h, g, d, C, N, M, I = !1) => {
    const m = r && r.children, B = r ? r.shapeFlag : 0, w = a.children, { patchFlag: Y, shapeFlag: O } = a;
    if (Y > 0) {
      if (Y & 128) {
        Ye(m, w, h, g, d, C, N, M, I);
        return;
      } else if (Y & 256) {
        Kt(m, w, h, g, d, C, N, M, I);
        return;
      }
    }
    O & 8 ? (B & 16 && kA(m, d, C), w !== m && f(h, w)) : B & 16 ? O & 16 ? Ye(m, w, h, g, d, C, N, M, I) : kA(m, d, C, !0) : (B & 8 && f(h, ""), O & 16 && z(w, h, g, d, C, N, M, I));
  }, Kt = (r, a, h, g, d, C, N, M, I) => {
    r = r || Ne, a = a || Ne;
    const m = r.length, B = a.length, w = Math.min(m, B);
    let Y;
    for (Y = 0; Y < w; Y++) {
      const O = a[Y] = I ? zA(a[Y]) : xA(a[Y]);
      y(r[Y], O, h, null, d, C, N, M, I);
    }
    m > B ? kA(r, d, C, !0, !1, w) : z(a, h, g, d, C, N, M, I, w);
  }, Ye = (r, a, h, g, d, C, N, M, I) => {
    let m = 0;
    const B = a.length;
    let w = r.length - 1, Y = B - 1;
    for (; m <= w && m <= Y; ) {
      const O = r[m], J = a[m] = I ? zA(a[m]) : xA(a[m]);
      if (te(O, J))
        y(O, J, h, null, d, C, N, M, I);
      else
        break;
      m++;
    }
    for (; m <= w && m <= Y; ) {
      const O = r[w], J = a[Y] = I ? zA(a[Y]) : xA(a[Y]);
      if (te(O, J))
        y(O, J, h, null, d, C, N, M, I);
      else
        break;
      w--, Y--;
    }
    if (m > w) {
      if (m <= Y) {
        const O = Y + 1, J = O < B ? a[O].el : g;
        for (; m <= Y; )
          y(null, a[m] = I ? zA(a[m]) : xA(a[m]), h, J, d, C, N, M, I), m++;
      }
    } else if (m > Y)
      for (; m <= w; )
        qA(r[m], d, C, !0), m++;
    else {
      const O = m, J = m, W = /* @__PURE__ */ new Map();
      for (m = J; m <= Y; m++) {
        const cA = a[m] = I ? zA(a[m]) : xA(a[m]);
        cA.key != null && (process.env.NODE_ENV !== "production" && W.has(cA.key) && x("Duplicate keys found during update:", JSON.stringify(cA.key), "Make sure keys are unique."), W.set(cA.key, m));
      }
      let F, tA = 0;
      const IA = Y - J + 1;
      let Ee = !1, Vo = 0;
      const Qe = new Array(IA);
      for (m = 0; m < IA; m++)
        Qe[m] = 0;
      for (m = O; m <= w; m++) {
        const cA = r[m];
        if (tA >= IA) {
          qA(cA, d, C, !0);
          continue;
        }
        let YA;
        if (cA.key != null)
          YA = W.get(cA.key);
        else
          for (F = J; F <= Y; F++)
            if (Qe[F - J] === 0 && te(cA, a[F])) {
              YA = F;
              break;
            }
        YA === void 0 ? qA(cA, d, C, !0) : (Qe[YA - J] = m + 1, YA >= Vo ? Vo = YA : Ee = !0, y(cA, a[YA], h, null, d, C, N, M, I), tA++);
      }
      const Jo = Ee ? Wr(Qe) : Ne;
      for (F = Jo.length - 1, m = IA - 1; m >= 0; m--) {
        const cA = J + m, YA = a[cA], To = cA + 1 < B ? a[cA + 1].el : g;
        Qe[m] === 0 ? y(null, YA, h, To, d, C, N, M, I) : Ee && (F < 0 || m !== Jo[F] ? de(YA, h, To, 2) : F--);
      }
    }
  }, de = (r, a, h, g, d = null) => {
    const { el: C, type: N, transition: M, children: I, shapeFlag: m } = r;
    if (m & 6) {
      de(r.component.subTree, a, h, g);
      return;
    }
    if (m & 128) {
      r.suspense.move(a, h, g);
      return;
    }
    if (m & 64) {
      N.move(r, a, h, ge);
      return;
    }
    if (N === gA) {
      o(C, a, h);
      for (let w = 0; w < I.length; w++)
        de(I[w], a, h, g);
      o(r.anchor, a, h);
      return;
    }
    if (N === at) {
      K(r, a, h);
      return;
    }
    if (g !== 2 && m & 1 && M)
      if (g === 0)
        M.beforeEnter(C), o(C, a, h), hA(() => M.enter(C), d);
      else {
        const { leave: w, delayLeave: Y, afterLeave: O } = M, J = () => o(C, a, h), W = () => {
          w(C, () => {
            J(), O && O();
          });
        };
        Y ? Y(C, J, W) : W();
      }
    else
      o(C, a, h);
  }, qA = (r, a, h, g = !1, d = !1) => {
    const { type: C, props: N, ref: M, children: I, dynamicChildren: m, shapeFlag: B, patchFlag: w, dirs: Y } = r;
    if (M != null && co(M, null, h, r, !0), B & 256) {
      a.ctx.deactivate(r);
      return;
    }
    const O = B & 1 && Y, J = !lt(r);
    let W;
    if (J && (W = N && N.onVnodeBeforeUnmount) && QA(W, a, r), B & 6)
      Os(r.component, h, g);
    else {
      if (B & 128) {
        r.suspense.unmount(h, g);
        return;
      }
      O && _A(r, null, a, "beforeUnmount"), B & 64 ? r.type.remove(r, a, h, d, ge, g) : m && (C !== gA || w > 0 && w & 64) ? kA(m, a, h, !1, !0) : (C === gA && w & 384 || !d && B & 16) && kA(I, a, h), g && kt(r);
    }
    (J && (W = N && N.onVnodeUnmounted) || O) && hA(() => {
      W && QA(W, a, r), O && _A(r, null, a, "unmounted");
    }, h);
  }, kt = (r) => {
    const { type: a, el: h, anchor: g, transition: d } = r;
    if (a === gA) {
      process.env.NODE_ENV !== "production" && r.patchFlag > 0 && r.patchFlag & 2048 && d && !d.persisted ? r.children.forEach((N) => {
        N.type === pA ? n(N.el) : kt(N);
      }) : Ss(h, g);
      return;
    }
    if (a === at) {
      q(r);
      return;
    }
    const C = () => {
      n(h), d && !d.persisted && d.afterLeave && d.afterLeave();
    };
    if (r.shapeFlag & 1 && d && !d.persisted) {
      const { leave: N, delayLeave: M } = d, I = () => N(h, C);
      M ? M(r.el, C, I) : I();
    } else
      C();
  }, Ss = (r, a) => {
    let h;
    for (; r !== a; )
      h = E(r), n(r), r = h;
    n(a);
  }, Os = (r, a, h) => {
    process.env.NODE_ENV !== "production" && r.type.__hmrId && Ki(r);
    const { bum: g, scope: d, update: C, subTree: N, um: M } = r;
    g && Se(g), d.stop(), C && (C.active = !1, qA(N, r, a, h)), M && hA(M, a), hA(() => {
      r.isUnmounted = !0;
    }, a), a && a.pendingBranch && !a.isUnmounted && r.asyncDep && !r.asyncResolved && r.suspenseId === a.pendingId && (a.deps--, a.deps === 0 && a.resolve()), process.env.NODE_ENV !== "production" && Fi(r);
  }, kA = (r, a, h, g = !1, d = !1, C = 0) => {
    for (let N = C; N < r.length; N++)
      qA(r[N], a, h, g, d);
  }, Xe = (r) => r.shapeFlag & 6 ? Xe(r.component.subTree) : r.shapeFlag & 128 ? r.suspense.next() : E(r.anchor || r.el), ko = (r, a, h) => {
    r == null ? a._vnode && qA(a._vnode, null, null, !0) : y(a._vnode || null, r, a, null, null, null, h), Ho(), vn(), a._vnode = r;
  }, ge = {
    p: y,
    um: qA,
    m: de,
    r: kt,
    mt: bA,
    mc: z,
    pc: yA,
    pbc: rA,
    n: Xe,
    o: A
  };
  let Vt, Jt;
  return e && ([Vt, Jt] = e(ge)), {
    render: ko,
    hydrate: Vt,
    createApp: Zr(ko, Vt)
  };
}
function $A({ effect: A, update: e }, t) {
  A.allowRecurse = e.allowRecurse = t;
}
function ct(A, e, t = !1) {
  const o = A.children, n = e.children;
  if (Q(o) && Q(n))
    for (let s = 0; s < o.length; s++) {
      const i = o[s];
      let l = n[s];
      l.shapeFlag & 1 && !l.dynamicChildren && ((l.patchFlag <= 0 || l.patchFlag === 32) && (l = n[s] = zA(n[s]), l.el = i.el), t || ct(i, l)), l.type === He && (l.el = i.el), process.env.NODE_ENV !== "production" && l.type === pA && !l.el && (l.el = i.el);
    }
}
function Wr(A) {
  const e = A.slice(), t = [0];
  let o, n, s, i, l;
  const c = A.length;
  for (o = 0; o < c; o++) {
    const u = A[o];
    if (u !== 0) {
      if (n = t[t.length - 1], A[n] < u) {
        e[o] = n, t.push(o);
        continue;
      }
      for (s = 0, i = t.length - 1; s < i; )
        l = s + i >> 1, A[t[l]] < u ? s = l + 1 : i = l;
      u < A[t[s]] && (s > 0 && (e[o] = t[s - 1]), t[s] = o);
    }
  }
  for (s = t.length, i = t[s - 1]; s-- > 0; )
    t[s] = i, i = e[i];
  return t;
}
const Lr = (A) => A.__isTeleport, gA = Symbol(process.env.NODE_ENV !== "production" ? "Fragment" : void 0), He = Symbol(process.env.NODE_ENV !== "production" ? "Text" : void 0), pA = Symbol(process.env.NODE_ENV !== "production" ? "Comment" : void 0), at = Symbol(process.env.NODE_ENV !== "production" ? "Static" : void 0), ke = [];
let NA = null;
function ae(A = !1) {
  ke.push(NA = A ? null : []);
}
function zr() {
  ke.pop(), NA = ke[ke.length - 1] || null;
}
let Fe = 1;
function rn(A) {
  Fe += A;
}
function Is(A) {
  return A.dynamicChildren = Fe > 0 ? NA || Ne : null, zr(), Fe > 0 && NA && NA.push(A), A;
}
function be(A, e, t, o, n, s) {
  return Is($(A, e, t, o, n, s, !0));
}
function xs(A, e, t, o, n) {
  return Is(wA(A, e, t, o, n, !0));
}
function Oo(A) {
  return A ? A.__v_isVNode === !0 : !1;
}
function te(A, e) {
  return process.env.NODE_ENV !== "production" && e.shapeFlag & 6 && Me.has(e.type) ? (A.shapeFlag &= -257, e.shapeFlag &= -513, !1) : A.type === e.type && A.key === e.key;
}
const Hr = (...A) => ws(...A), Dt = "__vInternal", Ns = ({ key: A }) => A ?? null, ut = ({ ref: A, ref_key: e, ref_for: t }) => A != null ? AA(A) || nA(A) || R(A) ? { i: mA, r: A, k: e, f: !!t } : A : null;
function $(A, e = null, t = null, o = 0, n = null, s = A === gA ? 0 : 1, i = !1, l = !1) {
  const c = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: A,
    props: e,
    key: e && Ns(e),
    ref: e && ut(e),
    scopeId: Ot,
    slotScopeIds: null,
    children: t,
    component: null,
    suspense: null,
    ssContent: null,
    ssFallback: null,
    dirs: null,
    transition: null,
    el: null,
    anchor: null,
    target: null,
    targetAnchor: null,
    staticCount: 0,
    shapeFlag: s,
    patchFlag: o,
    dynamicProps: n,
    dynamicChildren: null,
    appContext: null,
    ctx: mA
  };
  return l ? (Ro(c, t), s & 128 && A.normalize(c)) : t && (c.shapeFlag |= AA(t) ? 8 : 16), process.env.NODE_ENV !== "production" && c.key !== c.key && x("VNode created with invalid key (NaN). VNode type:", c.type), Fe > 0 && !i && NA && (c.patchFlag > 0 || s & 6) && c.patchFlag !== 32 && NA.push(c), c;
}
const wA = process.env.NODE_ENV !== "production" ? Hr : ws;
function ws(A, e = null, t = null, o = 0, n = null, s = !1) {
  if ((!A || A === mr) && (process.env.NODE_ENV !== "production" && !A && x(`Invalid vnode type when creating vnode: ${A}.`), A = pA), Oo(A)) {
    const l = UA(A, e, !0);
    return t && Ro(l, t), Fe > 0 && !s && NA && (l.shapeFlag & 6 ? NA[NA.indexOf(A)] = l : NA.push(l)), l.patchFlag |= -2, l;
  }
  if (Qs(A) && (A = A.__vccOpts), e) {
    e = vr(e);
    let { class: l, style: c } = e;
    l && !AA(l) && (e.class = fo(l)), b(c) && (_t(c) && !Q(c) && (c = _({}, c)), e.style = Ct(c));
  }
  const i = AA(A) ? 1 : er(A) ? 128 : Lr(A) ? 64 : b(A) ? 4 : R(A) ? 2 : 0;
  return process.env.NODE_ENV !== "production" && i & 4 && _t(A) && (A = U(A), x("Vue received a Component which was made a reactive object. This can lead to unnecessary performance overhead, and should be avoided by marking the component with `markRaw` or using `shallowRef` instead of `ref`.", `
Component that was made reactive: `, A)), $(A, e, t, o, n, i, s, !0);
}
function vr(A) {
  return A ? _t(A) || Dt in A ? _({}, A) : A : null;
}
function UA(A, e, t = !1) {
  const { props: o, ref: n, patchFlag: s, children: i } = A, l = e ? Pr(o || {}, e) : o;
  return {
    __v_isVNode: !0,
    __v_skip: !0,
    type: A.type,
    props: l,
    key: l && Ns(l),
    ref: e && e.ref ? t && n ? Q(n) ? n.concat(ut(e)) : [n, ut(e)] : ut(e) : n,
    scopeId: A.scopeId,
    slotScopeIds: A.slotScopeIds,
    children: process.env.NODE_ENV !== "production" && s === -1 && Q(i) ? i.map(Gs) : i,
    target: A.target,
    targetAnchor: A.targetAnchor,
    staticCount: A.staticCount,
    shapeFlag: A.shapeFlag,
    patchFlag: e && A.type !== gA ? s === -1 ? 16 : s | 16 : s,
    dynamicProps: A.dynamicProps,
    dynamicChildren: A.dynamicChildren,
    appContext: A.appContext,
    dirs: A.dirs,
    transition: A.transition,
    component: A.component,
    suspense: A.suspense,
    ssContent: A.ssContent && UA(A.ssContent),
    ssFallback: A.ssFallback && UA(A.ssFallback),
    el: A.el,
    anchor: A.anchor,
    ctx: A.ctx
  };
}
function Gs(A) {
  const e = UA(A);
  return Q(A.children) && (e.children = A.children.map(Gs)), e;
}
function Xr(A = " ", e = 0) {
  return wA(He, null, A, e);
}
function xA(A) {
  return A == null || typeof A == "boolean" ? wA(pA) : Q(A) ? wA(
    gA,
    null,
    A.slice()
  ) : typeof A == "object" ? zA(A) : wA(He, null, String(A));
}
function zA(A) {
  return A.el === null && A.patchFlag !== -1 || A.memo ? A : UA(A);
}
function Ro(A, e) {
  let t = 0;
  const { shapeFlag: o } = A;
  if (e == null)
    e = null;
  else if (Q(e))
    t = 16;
  else if (typeof e == "object")
    if (o & 65) {
      const n = e.default;
      n && (n._c && (n._d = !1), Ro(A, n()), n._c && (n._d = !0));
      return;
    } else {
      t = 32;
      const n = e._;
      !n && !(Dt in e) ? e._ctx = mA : n === 3 && mA && (mA.slots._ === 1 ? e._ = 1 : (e._ = 2, A.patchFlag |= 1024));
    }
  else
    R(e) ? (e = { default: e, _ctx: mA }, t = 32) : (e = String(e), o & 64 ? (t = 16, e = [Xr(e)]) : t = 8);
  A.children = e, A.shapeFlag |= t;
}
function Pr(...A) {
  const e = {};
  for (let t = 0; t < A.length; t++) {
    const o = A[t];
    for (const n in o)
      if (n === "class")
        e.class !== o.class && (e.class = fo([e.class, o.class]));
      else if (n === "style")
        e.style = Ct([e.style, o.style]);
      else if (We(n)) {
        const s = e[n], i = o[n];
        i && s !== i && !(Q(s) && s.includes(i)) && (e[n] = s ? [].concat(s, i) : i);
      } else
        n !== "" && (e[n] = o[n]);
  }
  return e;
}
function QA(A, e, t, o = null) {
  CA(A, e, 7, [
    t,
    o
  ]);
}
const _r = Cs();
let $r = 0;
function Al(A, e, t) {
  const o = A.type, n = (e ? e.appContext : A.appContext) || _r, s = {
    uid: $r++,
    vnode: A,
    type: o,
    parent: e,
    appContext: n,
    root: null,
    next: null,
    subTree: null,
    effect: null,
    update: null,
    scope: new Ws(!0),
    render: null,
    proxy: null,
    exposed: null,
    exposeProxy: null,
    withProxy: null,
    provides: e ? e.provides : Object.create(n.provides),
    accessCache: null,
    renderCache: [],
    components: null,
    directives: null,
    propsOptions: ds(o, n),
    emitsOptions: es(o, n),
    emit: null,
    emitted: null,
    propsDefaults: L,
    inheritAttrs: o.inheritAttrs,
    ctx: L,
    data: L,
    props: L,
    attrs: L,
    slots: L,
    refs: L,
    setupState: L,
    setupContext: null,
    suspense: t,
    suspenseId: t ? t.pendingId : 0,
    asyncDep: null,
    asyncResolved: !1,
    isMounted: !1,
    isUnmounted: !1,
    isDeactivated: !1,
    bc: null,
    c: null,
    bm: null,
    m: null,
    bu: null,
    u: null,
    um: null,
    bum: null,
    da: null,
    a: null,
    rtg: null,
    rtc: null,
    ec: null,
    sp: null
  };
  return process.env.NODE_ENV !== "production" ? s.ctx = Cr(s) : s.ctx = { _: s }, s.root = e ? e.root : s, s.emit = Li.bind(null, s), A.ce && A.ce(s), s;
}
let oA = null;
const el = () => oA || mA, Ge = (A) => {
  oA = A, A.scope.on();
}, ue = () => {
  oA && oA.scope.off(), oA = null;
}, tl = /* @__PURE__ */ Be("slot,component");
function ao(A, e) {
  const t = e.isNativeTag || In;
  (tl(A) || t(A)) && x("Do not use built-in or reserved HTML elements as component id: " + A);
}
function Bs(A) {
  return A.vnode.shapeFlag & 4;
}
let qe = !1;
function ol(A, e = !1) {
  qe = e;
  const { props: t, children: o } = A.vnode, n = Bs(A);
  Qr(A, t, n, e), Jr(A, o);
  const s = n ? nl(A, e) : void 0;
  return qe = !1, s;
}
function nl(A, e) {
  var t;
  const o = A.type;
  if (process.env.NODE_ENV !== "production") {
    if (o.name && ao(o.name, A.appContext.config), o.components) {
      const s = Object.keys(o.components);
      for (let i = 0; i < s.length; i++)
        ao(s[i], A.appContext.config);
    }
    if (o.directives) {
      const s = Object.keys(o.directives);
      for (let i = 0; i < s.length; i++)
        us(s[i]);
    }
    o.compilerOptions && sl() && x('"compilerOptions" is only supported when using a build of Vue that includes the runtime compiler. Since you are using a runtime-only build, the options should be passed via your build tool config instead.');
  }
  A.accessCache = /* @__PURE__ */ Object.create(null), A.proxy = Tn(new Proxy(A.ctx, ps)), process.env.NODE_ENV !== "production" && Ir(A);
  const { setup: n } = o;
  if (n) {
    const s = A.setupContext = n.length > 1 ? il(A) : null;
    Ge(A), fe();
    const i = jA(n, A, 0, [process.env.NODE_ENV !== "production" ? Ie(A.props) : A.props, s]);
    if (he(), ue(), Eo(i)) {
      if (i.then(ue, ue), e)
        return i.then((l) => {
          ln(A, l, e);
        }).catch((l) => {
          Yt(l, A, 0);
        });
      if (A.asyncDep = i, process.env.NODE_ENV !== "production" && !A.suspense) {
        const l = (t = o.name) !== null && t !== void 0 ? t : "Anonymous";
        x(`Component <${l}>: setup function returned a promise, but no <Suspense> boundary was found in the parent component tree. A component with async setup() must be nested in a <Suspense> in order to be rendered.`);
      }
    } else
      ln(A, i, e);
  } else
    ys(A, e);
}
function ln(A, e, t) {
  R(e) ? A.type.__ssrInlineRender ? A.ssrRender = e : A.render = e : b(e) ? (process.env.NODE_ENV !== "production" && Oo(e) && x("setup() should not return VNodes directly - return a render function instead."), process.env.NODE_ENV !== "production" && (A.devtoolsRawSetupState = e), A.setupState = Fn(e), process.env.NODE_ENV !== "production" && xr(A)) : process.env.NODE_ENV !== "production" && e !== void 0 && x(`setup() should return an object. Received: ${e === null ? "null" : typeof e}`), ys(A, t);
}
let uo;
const sl = () => !uo;
function ys(A, e, t) {
  const o = A.type;
  if (!A.render) {
    if (!e && uo && !o.render) {
      const n = o.template || Qo(A).template;
      if (n) {
        process.env.NODE_ENV !== "production" && VA(A, "compile");
        const { isCustomElement: s, compilerOptions: i } = A.appContext.config, { delimiters: l, compilerOptions: c } = o, u = _(_({
          isCustomElement: s,
          delimiters: l
        }, i), c);
        o.render = uo(n, u), process.env.NODE_ENV !== "production" && JA(A, "compile");
      }
    }
    A.render = o.render || iA;
  }
  Ge(A), fe(), wr(A), he(), ue(), process.env.NODE_ENV !== "production" && !o.render && A.render === iA && !e && (o.template ? x('Component provided template option but runtime compilation is not supported in this build of Vue. Configure your bundler to alias "vue" to "vue/dist/vue.esm-bundler.js".') : x("Component is missing template or render function."));
}
function cn(A) {
  return new Proxy(A.attrs, process.env.NODE_ENV !== "production" ? {
    get(e, t) {
      return Et(), dA(A, "get", "$attrs"), e[t];
    },
    set() {
      return x("setupContext.attrs is readonly."), !1;
    },
    deleteProperty() {
      return x("setupContext.attrs is readonly."), !1;
    }
  } : {
    get(e, t) {
      return dA(A, "get", "$attrs"), e[t];
    }
  });
}
function il(A) {
  const e = (o) => {
    process.env.NODE_ENV !== "production" && A.exposed && x("expose() should be called only once per setup()."), A.exposed = o || {};
  };
  let t;
  return process.env.NODE_ENV !== "production" ? Object.freeze({
    get attrs() {
      return t || (t = cn(A));
    },
    get slots() {
      return Ie(A.slots);
    },
    get emit() {
      return (o, ...n) => A.emit(o, ...n);
    },
    expose: e
  }) : {
    get attrs() {
      return t || (t = cn(A));
    },
    slots: A.slots,
    emit: A.emit,
    expose: e
  };
}
function Do(A) {
  if (A.exposed)
    return A.exposeProxy || (A.exposeProxy = new Proxy(Fn(Tn(A.exposed)), {
      get(e, t) {
        if (t in e)
          return e[t];
        if (t in ce)
          return ce[t](A);
      },
      has(e, t) {
        return t in e || t in ce;
      }
    }));
}
const rl = /(?:^|[-_])(\w)/g, ll = (A) => A.replace(rl, (e) => e.toUpperCase()).replace(/[-_]/g, "");
function Ys(A, e = !0) {
  return R(A) ? A.displayName || A.name : A.name || e && A.__name;
}
function Ut(A, e, t = !1) {
  let o = Ys(e);
  if (!o && e.__file) {
    const n = e.__file.match(/([^/\\]+)\.\w+$/);
    n && (o = n[1]);
  }
  if (!o && A && A.parent) {
    const n = (s) => {
      for (const i in s)
        if (s[i] === e)
          return i;
    };
    o = n(A.components || A.parent.type.components) || n(A.appContext.components);
  }
  return o ? ll(o) : t ? "App" : "Anonymous";
}
function Qs(A) {
  return R(A) && "__vccOpts" in A;
}
const cl = (A, e) => wi(A, e, qe), al = Symbol(process.env.NODE_ENV !== "production" ? "ssrContext" : ""), ul = () => {
  {
    const A = rt(al);
    return A || process.env.NODE_ENV !== "production" && x("Server rendering context not provided. Make sure to only call useSSRContext() conditionally in the server build."), A;
  }
};
function qt(A) {
  return !!(A && A.__v_isShallow);
}
function pl() {
  if (process.env.NODE_ENV === "production" || typeof window > "u")
    return;
  const A = { style: "color:#3ba776" }, e = { style: "color:#0b1bc9" }, t = { style: "color:#b62e24" }, o = { style: "color:#9d288c" }, n = {
    header(p) {
      return b(p) ? p.__isVue ? ["div", A, "VueInstance"] : nA(p) ? [
        "div",
        {},
        ["span", A, f(p)],
        "<",
        l(p.value),
        ">"
      ] : ie(p) ? [
        "div",
        {},
        ["span", A, qt(p) ? "ShallowReactive" : "Reactive"],
        "<",
        l(p),
        `>${PA(p) ? " (readonly)" : ""}`
      ] : PA(p) ? [
        "div",
        {},
        ["span", A, qt(p) ? "ShallowReadonly" : "Readonly"],
        "<",
        l(p),
        ">"
      ] : null : null;
    },
    hasBody(p) {
      return p && p.__isVue;
    },
    body(p) {
      if (p && p.__isVue)
        return [
          "div",
          {},
          ...s(p.$)
        ];
    }
  };
  function s(p) {
    const E = [];
    p.type.props && p.props && E.push(i("props", U(p.props))), p.setupState !== L && E.push(i("setup", p.setupState)), p.data !== L && E.push(i("data", U(p.data)));
    const G = c(p, "computed");
    G && E.push(i("computed", G));
    const D = c(p, "inject");
    return D && E.push(i("injected", D)), E.push([
      "div",
      {},
      [
        "span",
        {
          style: o.style + ";opacity:0.66"
        },
        "$ (internal): "
      ],
      ["object", { object: p }]
    ]), E;
  }
  function i(p, E) {
    return E = _({}, E), Object.keys(E).length ? [
      "div",
      { style: "line-height:1.25em;margin-bottom:0.6em" },
      [
        "div",
        {
          style: "color:#476582"
        },
        p
      ],
      [
        "div",
        {
          style: "padding-left:1.25em"
        },
        ...Object.keys(E).map((G) => [
          "div",
          {},
          ["span", o, G + ": "],
          l(E[G], !1)
        ])
      ]
    ] : ["span", {}];
  }
  function l(p, E = !0) {
    return typeof p == "number" ? ["span", e, p] : typeof p == "string" ? ["span", t, JSON.stringify(p)] : typeof p == "boolean" ? ["span", o, p] : b(p) ? ["object", { object: E ? U(p) : p }] : ["span", t, String(p)];
  }
  function c(p, E) {
    const G = p.type;
    if (R(G))
      return;
    const D = {};
    for (const y in p.ctx)
      u(G, y, E) && (D[y] = p.ctx[y]);
    return D;
  }
  function u(p, E, G) {
    const D = p[G];
    if (Q(D) && D.includes(E) || b(D) && E in D || p.extends && u(p.extends, E, G) || p.mixins && p.mixins.some((y) => u(y, E, G)))
      return !0;
  }
  function f(p) {
    return qt(p) ? "ShallowRef" : p.effect ? "ComputedRef" : "Ref";
  }
  window.devtoolsFormatters ? window.devtoolsFormatters.push(n) : window.devtoolsFormatters = [n];
}
const an = "3.2.45", fl = "http://www.w3.org/2000/svg", oe = typeof document < "u" ? document : null, un = oe && /* @__PURE__ */ oe.createElement("template"), hl = {
  insert: (A, e, t) => {
    e.insertBefore(A, t || null);
  },
  remove: (A) => {
    const e = A.parentNode;
    e && e.removeChild(A);
  },
  createElement: (A, e, t, o) => {
    const n = e ? oe.createElementNS(fl, A) : oe.createElement(A, t ? { is: t } : void 0);
    return A === "select" && o && o.multiple != null && n.setAttribute("multiple", o.multiple), n;
  },
  createText: (A) => oe.createTextNode(A),
  createComment: (A) => oe.createComment(A),
  setText: (A, e) => {
    A.nodeValue = e;
  },
  setElementText: (A, e) => {
    A.textContent = e;
  },
  parentNode: (A) => A.parentNode,
  nextSibling: (A) => A.nextSibling,
  querySelector: (A) => oe.querySelector(A),
  setScopeId(A, e) {
    A.setAttribute(e, "");
  },
  insertStaticContent(A, e, t, o, n, s) {
    const i = t ? t.previousSibling : e.lastChild;
    if (n && (n === s || n.nextSibling))
      for (; e.insertBefore(n.cloneNode(!0), t), !(n === s || !(n = n.nextSibling)); )
        ;
    else {
      un.innerHTML = o ? `<svg>${A}</svg>` : A;
      const l = un.content;
      if (o) {
        const c = l.firstChild;
        for (; c.firstChild; )
          l.appendChild(c.firstChild);
        l.removeChild(c);
      }
      e.insertBefore(l, t);
    }
    return [
      i ? i.nextSibling : e.firstChild,
      t ? t.previousSibling : e.lastChild
    ];
  }
};
function dl(A, e, t) {
  const o = A._vtc;
  o && (e = (e ? [e, ...o] : [...o]).join(" ")), e == null ? A.removeAttribute("class") : t ? A.setAttribute("class", e) : A.className = e;
}
function gl(A, e, t) {
  const o = A.style, n = AA(t);
  if (t && !n) {
    for (const s in t)
      po(o, s, t[s]);
    if (e && !AA(e))
      for (const s in e)
        t[s] == null && po(o, s, "");
  } else {
    const s = o.display;
    n ? e !== t && (o.cssText = t) : e && A.removeAttribute("style"), "_vod" in A && (o.display = s);
  }
}
const El = /[^\\];\s*$/, pn = /\s*!important$/;
function po(A, e, t) {
  if (Q(t))
    t.forEach((o) => po(A, e, o));
  else if (t == null && (t = ""), process.env.NODE_ENV !== "production" && El.test(t) && x(`Unexpected semicolon at the end of '${e}' style value: '${t}'`), e.startsWith("--"))
    A.setProperty(e, t);
  else {
    const o = ml(A, e);
    pn.test(t) ? A.setProperty(EA(o), t.replace(pn, ""), "important") : A[o] = t;
  }
}
const fn = ["Webkit", "Moz", "ms"], Wt = {};
function ml(A, e) {
  const t = Wt[e];
  if (t)
    return t;
  let o = TA(e);
  if (o !== "filter" && o in A)
    return Wt[e] = o;
  o = Nt(o);
  for (let n = 0; n < fn.length; n++) {
    const s = fn[n] + o;
    if (s in A)
      return Wt[e] = s;
  }
  return e;
}
const hn = "http://www.w3.org/1999/xlink";
function Ml(A, e, t, o, n) {
  if (o && e.startsWith("xlink:"))
    t == null ? A.removeAttributeNS(hn, e.slice(6, e.length)) : A.setAttributeNS(hn, e, t);
  else {
    const s = Ts(e);
    t == null || s && !Mn(t) ? A.removeAttribute(e) : A.setAttribute(e, s ? "" : t);
  }
}
function Cl(A, e, t, o, n, s, i) {
  if (e === "innerHTML" || e === "textContent") {
    o && i(o, n, s), A[e] = t ?? "";
    return;
  }
  if (e === "value" && A.tagName !== "PROGRESS" && !A.tagName.includes("-")) {
    A._value = t;
    const c = t ?? "";
    (A.value !== c || A.tagName === "OPTION") && (A.value = c), t == null && A.removeAttribute(e);
    return;
  }
  let l = !1;
  if (t === "" || t == null) {
    const c = typeof A[e];
    c === "boolean" ? t = Mn(t) : t == null && c === "string" ? (t = "", l = !0) : c === "number" && (t = 0, l = !0);
  }
  try {
    A[e] = t;
  } catch (c) {
    process.env.NODE_ENV !== "production" && !l && x(`Failed setting prop "${e}" on <${A.tagName.toLowerCase()}>: value ${t} is invalid.`, c);
  }
  l && A.removeAttribute(e);
}
function Il(A, e, t, o) {
  A.addEventListener(e, t, o);
}
function xl(A, e, t, o) {
  A.removeEventListener(e, t, o);
}
function Nl(A, e, t, o, n = null) {
  const s = A._vei || (A._vei = {}), i = s[e];
  if (o && i)
    i.value = o;
  else {
    const [l, c] = wl(e);
    if (o) {
      const u = s[e] = yl(o, n);
      Il(A, l, u, c);
    } else
      i && (xl(A, l, i, c), s[e] = void 0);
  }
}
const dn = /(?:Once|Passive|Capture)$/;
function wl(A) {
  let e;
  if (dn.test(A)) {
    e = {};
    let o;
    for (; o = A.match(dn); )
      A = A.slice(0, A.length - o[0].length), e[o[0].toLowerCase()] = !0;
  }
  return [A[2] === ":" ? A.slice(3) : EA(A.slice(2)), e];
}
let Lt = 0;
const Gl = /* @__PURE__ */ Promise.resolve(), Bl = () => Lt || (Gl.then(() => Lt = 0), Lt = Date.now());
function yl(A, e) {
  const t = (o) => {
    if (!o._vts)
      o._vts = Date.now();
    else if (o._vts <= t.attached)
      return;
    CA(Yl(o, t.value), e, 5, [o]);
  };
  return t.value = A, t.attached = Bl(), t;
}
function Yl(A, e) {
  if (Q(e)) {
    const t = A.stopImmediatePropagation;
    return A.stopImmediatePropagation = () => {
      t.call(A), A._stopped = !0;
    }, e.map((o) => (n) => !n._stopped && o && o(n));
  } else
    return e;
}
const gn = /^on[a-z]/, Ql = (A, e, t, o, n = !1, s, i, l, c) => {
  e === "class" ? dl(A, o, n) : e === "style" ? gl(A, t, o) : We(e) ? pt(e) || Nl(A, e, t, o, i) : (e[0] === "." ? (e = e.slice(1), !0) : e[0] === "^" ? (e = e.slice(1), !1) : Sl(A, e, o, n)) ? Cl(A, e, o, s, i, l, c) : (e === "true-value" ? A._trueValue = o : e === "false-value" && (A._falseValue = o), Ml(A, e, o, n));
};
function Sl(A, e, t, o) {
  return o ? !!(e === "innerHTML" || e === "textContent" || e in A && gn.test(e) && R(t)) : e === "spellcheck" || e === "draggable" || e === "translate" || e === "form" || e === "list" && A.tagName === "INPUT" || e === "type" && A.tagName === "TEXTAREA" || gn.test(e) && AA(t) ? !1 : e in A;
}
function Ol(A, e) {
  const t = ye(A);
  class o extends Uo {
    constructor(s) {
      super(t, s, e);
    }
  }
  return o.def = t, o;
}
const Rl = typeof HTMLElement < "u" ? HTMLElement : class {
};
class Uo extends Rl {
  constructor(e, t = {}, o) {
    super(), this._def = e, this._props = t, this._instance = null, this._connected = !1, this._resolved = !1, this._numberProps = null, this.shadowRoot && o ? o(this._createVNode(), this.shadowRoot) : (process.env.NODE_ENV !== "production" && this.shadowRoot && x("Custom element has pre-rendered declarative shadow root but is not defined as hydratable. Use `defineSSRCustomElement`."), this.attachShadow({ mode: "open" }), this._def.__asyncLoader || this._resolveProps(this._def));
  }
  connectedCallback() {
    this._connected = !0, this._instance || (this._resolved ? this._update() : this._resolveDef());
  }
  disconnectedCallback() {
    this._connected = !1, Ln(() => {
      this._connected || (mn(null, this.shadowRoot), this._instance = null);
    });
  }
  _resolveDef() {
    this._resolved = !0;
    for (let o = 0; o < this.attributes.length; o++)
      this._setAttr(this.attributes[o].name);
    new MutationObserver((o) => {
      for (const n of o)
        this._setAttr(n.attributeName);
    }).observe(this, { attributes: !0 });
    const e = (o, n = !1) => {
      const { props: s, styles: i } = o;
      let l;
      if (s && !Q(s))
        for (const c in s) {
          const u = s[c];
          (u === Number || u && u.type === Number) && (c in this._props && (this._props[c] = ht(this._props[c])), (l || (l = /* @__PURE__ */ Object.create(null)))[TA(c)] = !0);
        }
      this._numberProps = l, n && this._resolveProps(o), this._applyStyles(i), this._update();
    }, t = this._def.__asyncLoader;
    t ? t().then((o) => e(o, !0)) : e(this._def);
  }
  _resolveProps(e) {
    const { props: t } = e, o = Q(t) ? t : Object.keys(t || {});
    for (const n of Object.keys(this))
      n[0] !== "_" && o.includes(n) && this._setProp(n, this[n], !0, !1);
    for (const n of o.map(TA))
      Object.defineProperty(this, n, {
        get() {
          return this._getProp(n);
        },
        set(s) {
          this._setProp(n, s);
        }
      });
  }
  _setAttr(e) {
    let t = this.getAttribute(e);
    const o = TA(e);
    this._numberProps && this._numberProps[o] && (t = ht(t)), this._setProp(o, t, !1);
  }
  _getProp(e) {
    return this._props[e];
  }
  _setProp(e, t, o = !0, n = !0) {
    t !== this._props[e] && (this._props[e] = t, n && this._instance && this._update(), o && (t === !0 ? this.setAttribute(EA(e), "") : typeof t == "string" || typeof t == "number" ? this.setAttribute(EA(e), t + "") : t || this.removeAttribute(EA(e))));
  }
  _update() {
    mn(this._createVNode(), this.shadowRoot);
  }
  _createVNode() {
    const e = wA(this._def, _({}, this._props));
    return this._instance || (e.ce = (t) => {
      this._instance = t, t.isCE = !0, process.env.NODE_ENV !== "production" && (t.ceReload = (s) => {
        this._styles && (this._styles.forEach((i) => this.shadowRoot.removeChild(i)), this._styles.length = 0), this._applyStyles(s), this._instance = null, this._update();
      });
      const o = (s, i) => {
        this.dispatchEvent(new CustomEvent(s, {
          detail: i
        }));
      };
      t.emit = (s, ...i) => {
        o(s, i), EA(s) !== s && o(EA(s), i);
      };
      let n = this;
      for (; n = n && (n.parentNode || n.host); )
        if (n instanceof Uo) {
          t.parent = n._instance, t.provides = n._instance.provides;
          break;
        }
    }), e;
  }
  _applyStyles(e) {
    e && e.forEach((t) => {
      const o = document.createElement("style");
      o.textContent = t, this.shadowRoot.appendChild(o), process.env.NODE_ENV !== "production" && (this._styles || (this._styles = [])).push(o);
    });
  }
}
const Dl = {
  name: String,
  type: String,
  css: {
    type: Boolean,
    default: !0
  },
  duration: [String, Number, Object],
  enterFromClass: String,
  enterActiveClass: String,
  enterToClass: String,
  appearFromClass: String,
  appearActiveClass: String,
  appearToClass: String,
  leaveFromClass: String,
  leaveActiveClass: String,
  leaveToClass: String
};
rr.props;
const Ul = /* @__PURE__ */ _({ patchProp: Ql }, hl);
let En;
function Kl() {
  return En || (En = br(Ul));
}
const mn = (...A) => {
  Kl().render(...A);
};
function kl() {
  pl();
}
process.env.NODE_ENV !== "production" && kl();
class Vl {
  constructor(e = null) {
    me(this, "x");
    me(this, "y");
    me(this, "z");
    me(this, "w");
    this.x = 199011210, this.y = 198904210, this.z = 199606230, this.w = (e || Date.now()) % 4294967296;
  }
  next() {
    const e = this.x ^ this.x << 11;
    return this.x = this.y, this.y = this.z, this.z = this.w, this.w = this.w ^ this.w >>> 19 ^ (e ^ e >>> 8);
  }
  random() {
    return Math.abs(this.next() / 4294967296);
  }
}
class P {
  constructor(e) {
    me(this, "_Cells");
    this._Cells = e.map((t) => ({ ...t }));
  }
  get Cells() {
    return Object.freeze(this._Cells.map((e) => Object.freeze(e)));
  }
  Copy() {
    return new P(this._Cells);
  }
  Open(e) {
    let t = this.Copy();
    return t = this.fillOpen(e), t.IsGameOver() && t._Cells.forEach((o, n) => {
      o.Bomb && (t._Cells[n].Open = !0, t._Cells[n].Flag = !1);
    }), t;
  }
  ToggleFlag(e) {
    const t = this.Copy();
    return t._Cells[e].Open || (t._Cells[e].Flag = !t._Cells[e].Flag), t;
  }
  IsComplete() {
    for (const e of this._Cells)
      if (!e.Bomb && !e.Open)
        return !1;
    return !0;
  }
  IsGameOver() {
    for (const e of this._Cells)
      if (e.Bomb && e.Open)
        return !0;
    return !1;
  }
  Size() {
    return Math.sqrt(this._Cells.length);
  }
  OpenCount() {
    return this._Cells.filter((e) => e.Open).length;
  }
  BombCount() {
    return this._Cells.filter((e) => e.Bomb).length;
  }
  static GetRandomField(e, t, o = new Vl()) {
    const n = e * e, s = Array(n).fill(!1), i = Array(n).fill(!1), l = Array(n).fill(!1);
    for (let c = 0; c < t; c++)
      l[c] = !0;
    for (let c = n - 1; c >= 0; c--) {
      const u = Math.floor(o.random() * (c + 1)), f = l[c];
      l[c] = l[u], l[u] = f;
    }
    return new P(Array(n).fill(null).map((c, u) => ({
      Open: s[u],
      Flag: i[u],
      Bomb: l[u],
      Count: P.getBombCount(u, l, e)
    })));
  }
  static getBombCount(e, t, o) {
    const [n, s] = P.indexToXy(e, o);
    let i = 0;
    return [
      P.xyToIndexOrNaN(n - 1, s + 0, o),
      P.xyToIndexOrNaN(n + 1, s + 0, o),
      P.xyToIndexOrNaN(n + 0, s - 1, o),
      P.xyToIndexOrNaN(n + 0, s + 1, o),
      P.xyToIndexOrNaN(n + 1, s + 1, o),
      P.xyToIndexOrNaN(n - 1, s - 1, o),
      P.xyToIndexOrNaN(n + 1, s - 1, o),
      P.xyToIndexOrNaN(n - 1, s + 1, o)
    ].filter((l) => !isNaN(l)).forEach((l) => {
      t[l] && i++;
    }), i;
  }
  static xyToIndex(e, t, o) {
    if (e < 0 || e >= o || t < 0 || t >= o)
      throw "Fail to convert xy to index";
    return e + t * o;
  }
  static xyToIndexOrNaN(e, t, o) {
    return e < 0 || e >= o || t < 0 || t >= o ? NaN : P.xyToIndex(e, t, o);
  }
  static indexToXy(e, t) {
    return [e % t, Math.floor(e / t)];
  }
  fillOpen(e) {
    const t = this.Copy(), o = [];
    for (o.push(e), t._Cells[e].Open = !0, t._Cells[e].Flag = !1; o.length > 0; ) {
      const n = o.pop();
      n !== void 0 && t._Cells[n].Count === 0 && P.getOpenableAdjacentIndex(t, n).forEach((s) => {
        t._Cells[s].Open = !0, t._Cells[s].Flag = !1, o.push(s);
      });
    }
    return t;
  }
  static getOpenableAdjacentIndex(e, t) {
    const [o, n] = P.indexToXy(t, e.Size());
    return [
      P.xyToIndexOrNaN(o - 1, n + 0, e.Size()),
      P.xyToIndexOrNaN(o + 1, n + 0, e.Size()),
      P.xyToIndexOrNaN(o + 0, n - 1, e.Size()),
      P.xyToIndexOrNaN(o + 0, n + 1, e.Size())
    ].filter((s) => !isNaN(s)).filter((s) => !e._Cells[s].Bomb && !e._Cells[s].Flag && !e._Cells[s].Open);
  }
}
const Jl = ["x", "y", "width", "height"], Tl = ["x", "y"], jl = ["display"], Zl = ["x", "y", "width", "height"], Fl = ["x1", "y1", "x2", "y2", "stroke"], bl = ["x1", "y1", "x2", "y2", "stroke"], ql = ["x1", "y1", "x2", "y2", "stroke"], Wl = ["x1", "y1", "x2", "y2", "stroke"], Ll = ["display", "x", "y"], zl = /* @__PURE__ */ ye({
  __name: "CellElement",
  props: {
    cell: null,
    x: null,
    y: null,
    cellSize: null,
    selected: { type: Boolean }
  },
  setup(A) {
    const e = A, t = parseInt(e.cellSize.toString()), o = parseInt(e.x.toString()), n = parseInt(e.y.toString()), s = () => e.cell.Bomb ? "💣" : e.cell.Count === 0 ? "" : e.cell.Count.toString();
    return (i, l) => (ae(), be("g", null, [
      $("rect", {
        x: S(o),
        y: S(n),
        width: S(t),
        height: S(t),
        stroke: "black",
        fill: "whitesmoke"
      }, null, 8, Jl),
      $("text", {
        x: S(o) + S(t) / 2,
        y: S(n) + S(t) / 2,
        "text-anchor": "middle",
        "dominant-baseline": "central",
        stroke: "black"
      }, ot(s()), 9, Tl),
      $("g", {
        display: e.cell.Open ? "none" : "block"
      }, [
        $("rect", {
          x: S(o),
          y: S(n),
          width: S(t),
          height: S(t),
          stroke: "black",
          fill: "lightgray"
        }, null, 8, Zl),
        $("line", {
          x1: S(o) + 0,
          y1: S(n) + 0,
          x2: S(o) + S(t),
          y2: S(n) + 0,
          stroke: e.selected ? "blue" : "white",
          "stroke-width": "3"
        }, null, 8, Fl),
        $("line", {
          x1: S(o) + S(t) - 3,
          y1: S(n) + 0,
          x2: S(o) + S(t) - 3,
          y2: S(n) + S(t),
          stroke: e.selected ? "blue" : "gray",
          "stroke-width": " 3"
        }, null, 8, bl),
        $("line", {
          x1: S(o) + S(t) - 3,
          y1: S(n) + S(t) - 3,
          x2: S(o) + 0,
          y2: S(n) + S(t) - 3,
          stroke: e.selected ? "blue" : "gray",
          "stroke-width": "3"
        }, null, 8, ql),
        $("line", {
          x1: S(o) + 0,
          y1: S(n) + S(t),
          x2: S(o) + 0,
          y2: S(n) + 0,
          stroke: e.selected ? "blue" : "white",
          "stroke-width": "3"
        }, null, 8, Wl)
      ], 8, jl),
      $("text", {
        display: e.cell.Flag ? "block" : "none",
        x: S(o) + S(t) / 2,
        y: S(n) + S(t) / 2,
        "text-anchor": "middle",
        "dominant-baseline": "central",
        stroke: "black"
      }, "🚩", 8, Ll)
    ]));
  }
}), Hl = /* @__PURE__ */ ye({
  __name: "FieldElement",
  props: {
    field: null,
    clicked: null,
    cellSize: null,
    index: null,
    selected: { type: Boolean }
  },
  setup(A) {
    const e = A, t = e.cellSize, o = e.field.Size() * t, n = (c) => {
      const u = c.offsetX, f = c.offsetY;
      window.matchMedia("(hover: none) and (pointer: coarse)").matches ? i(u, f, "menu") : i(u, f, c.button === 0 ? "open" : "flag"), c.preventDefault();
    }, s = (c) => {
      const u = c.offsetX, f = c.offsetY;
      return i(u, f, "flag"), c.preventDefault(), !1;
    }, i = (c, u, f) => {
      const p = Math.floor(c / t) + Math.floor(u / t) * e.field.Size();
      e.clicked(p, f);
    }, l = () => e.field.Cells.map((c, u) => {
      const f = u % e.field.Size() * t, p = Math.floor(u / e.field.Size()) * t;
      return { x: f, y: p, cell: c, index: u };
    });
    return (c, u) => (ae(), be("svg", {
      width: o,
      height: o,
      onClick: n,
      onContextmenu: s
    }, [
      (ae(!0), be(gA, null, Mr(l(), (f) => (ae(), xs(zl, {
        key: f.index,
        cell: f.cell,
        x: f.x,
        y: f.y,
        cellSize: S(t),
        selected: e.selected && e.index === A.index
      }, null, 8, ["cell", "x", "y", "cellSize", "selected"]))), 128)),
      $("rect", {
        x: "0",
        y: "0",
        width: o,
        height: o,
        opacity: "0"
      })
    ], 32));
  }
});
const Ko = (A, e) => {
  const t = A.__vccOpts || A;
  for (const [o, n] of e)
    t[o] = n;
  return t;
}, vl = /* @__PURE__ */ Ko(Hl, [["__scopeId", "data-v-44ff6dc0"]]), Xl = (A) => (zi("data-v-9c660bce"), A = A(), Hi(), A), Pl = /* @__PURE__ */ Xl(() => /* @__PURE__ */ $("div", { className: "title_bar" }, [
  /* @__PURE__ */ $("div", { className: "title" }, " Minesweeper ")
], -1)), _l = { class: "item num" }, $l = { class: "item center" }, Ac = { class: "item num" }, ec = /* @__PURE__ */ ye({
  __name: "BarElement",
  props: {
    field: null,
    clickReset: null,
    cellSize: null,
    time: null
  },
  setup(A) {
    const e = A, t = e.cellSize, o = () => e.field.IsComplete() ? "😎" : e.field.IsGameOver() ? "🥺" : "😀";
    return (n, s) => (ae(), be("div", null, [
      Pl,
      $("div", {
        style: Ct({ width: e.field.Size() * S(t) }),
        class: "bar"
      }, [
        $("div", _l, [
          $("b", null, ot(e.field.BombCount()), 1)
        ]),
        $("div", $l, [
          $("div", {
            class: "button",
            onClick: s[0] || (s[0] = () => {
              e.clickReset();
            })
          }, ot(o()), 1)
        ]),
        $("div", Ac, [
          $("b", null, ot(e.time), 1)
        ])
      ], 4)
    ]));
  }
});
const tc = /* @__PURE__ */ Ko(ec, [["__scopeId", "data-v-9c660bce"]]), oc = "data:audio/mpeg;base64,", nc = /* @__PURE__ */ ye({
  __name: "GameElement",
  props: {
    width: null,
    bomb: null,
    cellSize: null
  },
  setup(A) {
    const e = A, t = new Audio(oc), o = Oe(P.GetRandomField(e.width, e.bomb)), n = Oe(!1), s = Oe(0), i = Oe(0), l = Oe(0), c = () => {
      l.value = setInterval(() => {
        i.value += 1;
      }, 1e3);
    }, u = () => {
      i.value = 0, clearInterval(l.value);
    }, f = () => {
      clearInterval(l.value);
    }, p = () => {
      t.playbackRate = 1.5, t.play();
    }, E = () => {
      u(), o.value = P.GetRandomField(10, 10);
    }, G = (D, y) => {
      if (!o.value.IsGameOver() && !o.value.IsComplete())
        if (s.value = D, y === "open") {
          o.value.OpenCount() === 0 && c();
          const T = o.value.Open(D);
          o.value = T, (T.IsGameOver() || T.IsComplete()) && (f(), T.IsComplete() && p()), n.value = !1;
        } else
          y === "menu" ? o.value.Cells[D].Open || (n.value = !n.value) : (o.value = o.value.ToggleFlag(D), n.value = !1);
    };
    return (D, y) => (ae(), be(gA, null, [
      wA(tc, {
        field: S(o),
        clickReset: () => {
          E();
        },
        cellSize: A.cellSize,
        time: i.value
      }, null, 8, ["field", "clickReset", "cellSize", "time"]),
      wA(vl, {
        field: S(o),
        clicked: G,
        cellSize: A.cellSize,
        index: s.value,
        selected: n.value
      }, null, 8, ["field", "cellSize", "index", "selected"])
    ], 64));
  }
}), sc = /* @__PURE__ */ ye({
  __name: "MineSweeper.ce",
  props: {
    width: null,
    bomb: null,
    cellSize: null
  },
  setup(A) {
    const e = A;
    return (t, o) => (ae(), xs(nc, {
      width: e.width,
      bomb: e.bomb,
      cellSize: e.cellSize
    }, null, 8, ["width", "bomb", "cellSize"]));
  }
}), ic = `@font-face{font-family:DSEG7Classic-Bold;font-style:normal;font-weight:400;font-display:swap;src:local("DSEG7Classic-Bold"),url(data:font/woff2;base64,) format("woff2")}.bar{background-color:#d3d3d3;width:500px;display:flex;border-style:ridge;border-width:5px}.title_bar{width:500px;border-style:ridge;border-width:5px;background-color:#00f;display:flex}.title{width:50%;color:#fff;font-weight:bolder;padding-top:5px}.help{color:#000;margin-left:auto;font-weight:bolder}.item{width:calc(100% / 3);text-align:center;list-style:none;border-style:ridge;border-width:2px}.num{background-color:#321e1e;color:red;text-align:right;padding-right:5px;padding-top:5px;font-family:DSEG7Classic-Bold,sans-serif;font-size:18px}.center{display:flex;justify-content:center}.button{appearance:none;border-style:solid;border-width:3px;background-color:#d3d3d3;border-top-color:#f5f5f5;border-left-color:#f5f5f5;border-bottom-color:gray;border-right-color:gray;width:30px}svg{border-style:ridge;border-width:5px}
`, rc = /* @__PURE__ */ Ko(sc, [["styles", [ic]]]), cc = Ol(rc);
export {
  cc as mine_sweeper
};
